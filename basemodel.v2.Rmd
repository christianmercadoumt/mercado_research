...
re.tree.1 <- readRDS('data/model_objects/re.tree.1.rds')
```
...
re.plot.1 <- readRDS('data/model_objects/re.plot.1.rds')
re.tree.1 <- readRDS('data/model_objects/re.tree.1.rds')
...
for(i in 1:length(re.tree.1$var.summary)){
  par(mfrow = c(1,2))
  plot.gam(site5a.2, scheme = 1, residuals = T, rug = F, ylim = c(-5, 4), select = i)
  plot.gam(re.tree.1, scheme = 1, residuals = T, rug = F, ylim = c(-5, 4), select = i)
}
