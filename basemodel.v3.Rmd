...
function(){
  sm1 <- smooth_estimates(gam1) %>% add_confint()
  sm2 <- smooth_estimates(gam2) %>% add_confint() %>% rename(est2 = est, var2 = var, lwr2 = lower_ci, upr2 = upper_ci) %>% select(var2, est2, lwr2, upr2)
  df <- tibble(sm1, sm2)
  
}


### check that I can plot ggplot objects from separate data on the same plot
