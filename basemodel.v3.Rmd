...
model <- c('null', 'size', 'density.comp', 'site', 're')

rmse.sel.alt <- c(my.rmse.2(gam.null)[[2]], my.rmse.2(gam.s1a)[[2]], my.rmse.2(compd.ba1)[[2]], my.rmse.2(site.n5b)[[2]], my.rmse.2(re.tree.1alt)[[2]])

rmse.best1 <- c(my.rmse.2(gam.null)[[2]], my.rmse.2(gam.s1a)[[2]], my.rmse.2(best.6a)[[2]], my.rmse.2(site.n5a)[[2]], NA)

fvs <- c(my.rmse.2(gam.null)[[2]], my.rmse.2(fvs.size1)[[2]], my.rmse.2(fvs.sz.cmp)[[2]], my.rmse.2(fvs.full)[[2]], NA)

tibble(model, rmse.sel.alt, rmse.best1, fvs) #rmse.selected.old

rmse.selected <- c(my.rmse.2(gam.null)[[2]], my.rmse.2(gam.s1a)[[2]], my.rmse.2(compd.pl4)[[2]], my.rmse.2(site5a.2)[[2]], my.rmse.2(re.tree.1)[[2]])

tibble(model, rmse.selected, rmse.sel.alt)

```

```{r read in re model objects, include = F}
...
rmse.conc.table(list('stand' = re.stand.1alt, 'clust' = re.cluster.1alt, 'tree' = re.tree.1alt))

```


```{r fit previous models, include = F}
...
(low.res <- tibble(rownames_to_column(re.tree.1$model[which(res.tree %in% head(sort(res.tree), 50)),]), dev.res = head(sort(res.tree), )))
...
all.w.res <- tibble(cbind(re.tree.1$model, res.tree))
summary(res.tree)

all.res.case <- all.w.res %>% mutate(res.case=case_when(res.tree < -1.2 ~ 'low',
                                        res.tree > 1 ~ 'high'))


all.res.full <- left_join(bai.site.ids, all.res.case)

###########NOTE 
######## I NEED TO make a tibble that works here which allows me to plot through variables and get to the bottom of the deviance residuals stuff.

ggplot(all.w.res, aes(x = aspect_deg, y = res.tree, color = res.case)) + geom_point()
```

```{r residual inquiries}

ggplot(all.res.full, aes(x = aspect_deg, y = res.tree, color = res.case)) + geom_point()

ggplot(subset(all.res.full, !is.na(res.case)), aes(x = DIAMETER, y = res.tree, color = res.case)) + geom_point()

ggplot(subset(all.res.full, !is.na(res.case)), aes(x = DIAMETER, y = res.tree, color = unique_tree_id)) + geom_point() + facet_wrap(~res.case)

ggplot(subset(all.res.full, !is.na(res.case)), aes(x = grwth.yr, y = res.tree, color = res.case)) + geom_point()

 ggplot(all.res.full, aes(x = DIAMETER, y = res.tree, color = res.case)) + geom_point() + facet_wrap(~stand)



unique(all.res.full$habclass)
```
...
smooth_estimates(re.tree.1, 's(DIAMETER)')

```

```{r manual plots}
#smooth_estimates(re.tree.1)
sm.1 <- smooth_estimates(gam.s1a, n = 100) %>% add_confint()

sm.1a <- smooth_estimates(compd.pl4, n=100) %>% add_confint()

ggplot() + 
  geom_ribbon(data = sm.1, aes(x = DIAMETER, ymin = lower_ci, ymax = upper_ci), alpha = 0.2, color = 'lightblue') + 
  geom_ribbon(data = sm.1a, aes(x = DIAMETER, ymin = lower_ci, ymax = upper_ci), alpha = 0.2, color = 'pink') + 
  geom_line(data = sm.1, aes(x = DIAMETER, y = est), color = 'lightblue') + 
  geom_line(data = sm.1a, aes(x = DIAMETER, y = est), color = 'pink')

sm2 <- smooth_estimates(compd.pl4, n = 100, smooth = 's(DIAMETER)') %>% add_confint() %>% rename(est2 = est, d2 = DIAMETER ,lwr2 = lower_ci, upr2 = upper_ci) %>% select(d2, lwr2, upr2, est2)
sm3 <- smooth_estimates(site5a.2, n = 100, smooth = 's(DIAMETER)') %>% add_confint() %>% rename(est3 = est, d3 = DIAMETER, lwr3 = lower_ci, upr3 = upper_ci) %>% select(d3, est3, lwr3, upr3)
sm4 <- smooth_estimates(re.tree.1, n = 100, smooth = 's(DIAMETER)') %>% add_confint() %>% rename(est4 = est, d4 = DIAMETER, lwr4 = lower_ci, upr4 = upper_ci) %>% select(d4, lwr4, upr4, est4)

df <- tibble(sm.1, sm2)

ggplot(df) + geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, x = DIAMETER), alpha = 0.2, color = 'lightblue2') + geom_ribbon(aes(ymin = lwr2, ymax = upr2, x = d2), alpha = 0.2, color = 'pink') + geom_line(aes(x = DIAMETER, y = est), color = 'lightblue') + geom_line(aes(x = d2, y = est2), color = 'red')

df2 <- tibble(sm2, sm3)

ggplot(df2) + geom_ribbon(aes(ymin = lwr2, ymax = upr2, x = d2), alpha = 0.2, color = 'pink') + geom_ribbon(aes(ymin = lwr3, ymax = upr3, x = d3), alpha = 0.2, color = 'peachpuff') + geom_line(aes(x = d2, y = est2), color = 'red') + geom_line(aes(x = d3, y = est3), color = 'peachpuff')

df3 <- tibble(sm3, sm4)

ggplot(df3) + geom_ribbon(aes(ymin = lwr4, ymax = upr4, x = d4), alpha = 0.2, color = 'green') + geom_ribbon(aes(ymin = lwr3, ymax = upr3, x = d3), alpha = 0.2, color = 'peachpuff') + geom_line(aes(x = d4, y = est4), color = 'green') + geom_line(aes(x = d3, y = est3), color = 'peachpuff')


```

```{r write fn for smooth plotting}
# function(model, ){
#   require(mgcv)
#   require(gratia)
#   smooth.est <- smooth_estimates(model) %>% add_confint()
#   
# }

a <- list(site.n5a, site.n5b, best.6a)
aa <- lapply(a, smooth_estimates)


########## I might just have to do this manually - create list of model objects, then get their smooth estimates using lapply
########## Then build ggplot objects

# function(sm.est, variable, ribbon.opacity = 0.2){
#   ggplot() + 
#     geom_ribbon(data = sm.est, aes(x = variable, ymin = lower_ci, ymax = upper_ci), alpha = ribbon.opacity) +
#     geom_line(data = sm.est, aes(x = variable, y = est))
# }
...
### figure out if there is an easier way to select which smooth's data to extract
