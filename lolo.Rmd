...
##live_trees <- function(year, cluster){
  
##a <- 
lolo1a %>% select(SETTING_ID, PLOT, myear, LIVE_DEAD, TPA_EQUIV) %>% group_by(SETTING_ID, PLOT, myear, LIVE_DEAD, tpa = mean(TPA_EQUIV, na.rm = T)) %>% summarize(LIVE_DEAD = str_count(LIVE_DEAD, "L")) %>% summarize(LIVE_DEAD = sum(LIVE_DEAD))

##ggplot(a, aes(x = myear, y = LIVE_DEAD, color = SETTING_ID)) + geom_point()
#}

#The above presents a tibble which shows the count of live trees in a plot cluster at a given year. Current issue: the average tpa column is skewed by subplot data
