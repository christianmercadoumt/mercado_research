...
#simulated data
DIAMETER <- sample(seq(1, 25, 0.1), 100, replace = T)
...
asp_sin <- rep(-0.08693, 100)
asp_cos <- rep(0.6327, 100)

#varying diameter values - everything else constant
unique_tree_id <- sample(seq(2000, 3000, 1), 100)
...
hist(laoc_laoc$bai)
summary(sim.diam$DIAMETER)
sim.diam <- tibble(DIAMETER, cr, bal.pl.ratio, ba.pl, asp_sin, asp_cos, unique_tree_id)

bai.diam <- predict(laoc.gam$gam, newdata = sim.diam)
a <- tibble(sim.diam, bai = as.vector(bai.diam))

bai.diam <- predict(psme.gam$gam, newdata = sim.diam)
b <- tibble(sim.diam, bai = as.vector(bai.diam))

bai.diam <- predict(pico.gam$gam, newdata = sim.diam)
c <- tibble(sim.diam, bai = as.vector(bai.diam))

rm(DIAMETER)
ggplot() + geom_point(data = a, aes(x = DIAMETER, y = bai), color = 'red') + 
