...
Additive models (e.g., GAMs) are becoming more prevalent in ecological research 
to aid in capturing and describing complex non-linear 
relationships that otherwise would require extensive testing to determine a 
suitable parametric model form \citep{Robinson2011,Adamec2015}. 
...
\cite{Vospernik2021} such that BAI increases rapidly at lower 
...
trees with longer crowns have enhanced growth. The density 
effect of BAH was characterized as a decreasing effect approaching a negative 
limit (Fig. \ref{fig:basesmooths}), which is similar to that of \cite{Hann2003} 
\cite[displayed by][]{Weiskittel2011}. The agreement of these results is somewhat 
unsurprising given these studies strongly motivated the choice of variables considered here and 
...
The way that site conditions were represented in the selected model 
contrast from previous formulations, with the latter often including 
...
including a more comprehensive heatload metric based on a combination 
of slope, aspect, elevation, and incoming solar radiation \citep{Theobald2015}. 
Climate-based metrics (such as heatload) derived at varying cartographic scales 
...
and biophysical conditions are constant at smaller scales. \cite{Stage2007} suggested that 
using a combination of slope, aspect, and elevation should improve prediction 
accuracy, which was observed here. However, the realized improvements 
...
%  and that models estimating the variable 
% are accurate for the entire area estimated
% (i) the estimated 
...
(displayed in Fig. \ref{fig:basesmooths}), others have identified BAL with 
a concave-shaped decreasing effect on growth \citep{Monserud1996,Hann2003,Vospernik2021}. 
Of note, however, 
is that the present study implemented BAL as a ratio of plot BAH, similar to 
\cite{Stage1973}, but dissimilar to \cite{Monserud1996} and \cite{Vospernik2021}, 
who implemented BAL as an absolute quantity instead of a ratio (see \cite{Kiernan2008} for 
...
with species composition \citep{Brunner2020}. \cite{Contreras2011} identified 
BAL to have a closer relationship with growth in western larch than other 
distance-independent measures, partially justifying its use within the SCPt model. 
Additionally, this study identified important effects of BAL and BAH within the 
same model, which is consistent with \cite{Wykoff1990}, \cite{Monserud1996}, 
and more recently \cite{Riofrio2019} and \cite{Vospernik2021}.


% The selection process 
% implemented in this study found BAL to
% 
% However, the individual predictor 
...
% 
% 
% Despite the individual performance of BAL, however, 
% 
...
To realize the benefits of using a (non-parametric) smoother-based approach, 
one must ensure that a smooth has enough flexibility to capture complex 
functional forms, but that it does not capture \emph{too much} variability in 
observed scatter around those forms. The former requires setting a sufficiently 
...
that variability is penalized appropriately, which is modulated by the estimated 
smoothing parameter \citep[see section \ref{gamoverview}; ][]{wood2006generalized,Pedersen2019}. 
...
densely packed stands \citep{Reukema1979}. 
...
This study did not specifically evaluate competition for light, but rather identified 
...
However, the results provide evidence that the 
...
of Fig. \ref{fig:sumfig} do not display the same reduced growth shown 
in the relationships of other mixtures at higher diameters. Thus growth at larger values 
of DBH in Fig. \ref{fig:diffsmooths}, which show positive differences between the 
lodgepole pine mixtures and the pure larch smooths, should be questioned, and 
interpretation at values larger than 20 cm is withheld completely. Additionally, 
the results show negative growth differences at lower DBH values. This suggest 
that smaller-diameter western larch trees mixed with shade intolerant lodgepole 
pine experience lower rates of growth than the same size trees growing primarily 
with other western larch. Although not explicitly stated, this is 
in concurrence with the expectations, provided that lower growth in 
less shade-tolerant mixtures was expected. However, given that western larch and 
lodgepole pine have the same shade-intolerance value (of 1), the expectation was 
that there would not be any difference in growth between the corresponding 
mixtures. This may indicate that the competition 
for some resource other than light impacts the growth of smaller western larch 
trees in this mixture. Further research evaluating whether these small-tree 
negative growth effects are modulated by different proportions of each 
species in this mixture could help explain the presently observed effects. 


% , competitive position, relative size
% 
% across different levels of mixing between 
% western larch and lodgepole pine could elucidate the presently observed effects.
% 
% the negative growth effect 
% for smaller western larch trees in mixtures with lodgepole pine 
% 
% these shade-intolerant species is enhanced 
% 
% , these results are unexpected 
% such that there was \emph{negative} growth in lodgepole pine mixtures compared to 
% pure larch stands as opposed to there being no difference. 
% 
% tolerance of lodgepole pine is the same as western larch, but lower growth 
% was seen
...
area in larger trees as opposed to the absolute amount of the former, 
an issue of scale was created when comparing across mixtures. 
For example, if comparing a BAL value of 0.25 
in a pure larch plot where the BAH is 40 $\mathrm{m^2/ha}$, to a larch-lodgepole pine 
mixed plot where the BAH is 10 $\mathrm{m^2/ha}$, the tree of interest is facing
10 $\mathrm{m^2/ha}$ and 2.5 $\mathrm{m^2/ha}$ worth of area occupied by larger trees, 
...
western larch in mixtures with lodgepole pine are smaller (based on average 
and maximum DBH) than those in the pure data (as noted above). 
Therefore a competitively dominant larch 
...
the primary BAH effect (for the SCPt model, shown in Fig. \ref{fig:basesmooths}) is near its lower 
limit at 25 $\mathrm{m^2/ha}$, the negative growth differences for each mixture 
may simply imply that resources are generally scarce, leading to lower rates of growth. Thus 
there don't appear to be any complementary effects between western larch and 
...
characteristics on these aspects. As shown in Figure \ref{fig:aspel}, 
larch-Douglas-fir mixtures evaluated in this study were primarily on NW-facing aspects. 
...
However, since the growth of Douglas-fir trees was not evaluated, further 
...
with CCF (Table \ref{tab:mixvarsrmse}). These findings are in line with those of \cite{Dirnberger2017}, 
...
species dynamics are at play, as it only represents potential crown area as a weighted density. 
A comparison of CCF effects on western larch growth between specific 
mixtures could show how the CCF effect changes with mixture, however, that 
...
larch growth and both purity of larch and relative abundance of shade-intolerant 
species exists. Others have attributed differences in productivity to shade-tolerance 
...
modeling, given its ease of use, availability, and relevance, as suggested also by 
\cite{Russell2013}. 

\section{Management and other implications}

The information provided here may be useful when managing 
western larch in a multi-species setting. \cite{Crotteau2019} discuss 
...
to pure stands of either. However, \cite{Xie2020} assessed the height-diameter 
...
% impacted by species-mixing effects
