...
individual tree growth models. The effect of diameter on growth 
resembled the findings of \cite{Wykoff1990}, and 
\cite{Monserud1996}, such that growth increases rapidly at lower 
values of DBH and then taper at high values, but differs in that the BAI-DBH 
relationship modeled in this study does not show 
a decreasing effect \ref{fig:basesmooths}. With respect to the overall 
increasing effect and the lack of a decreasing effect, 
these results are most similar to \emph{L. decidua} presented by 
\cite{Vospernik2021}. 
The effects of crown ratio were similar to those shown by 
\cite{Hann2003} and \cite{Wykoff1990}, as it is represented by 
a near-linear increasing effect \ref{fig:basesmooths}. The density effect (BAH) was characterized 
with a decreasing effect approaching a negative effect limit \ref{fig:basesmooths}, which is 
similar to that of \cite{Hann2003} (shown in \cite{Weiskittel2011}). 
...
of slope, aspect, elevation, and incoming solar radiation \cite{Theobald2015}. 
Additionally, using climate-based metrics derived at varying cartographic scales 
rely on their own set of assumptions. Of particular relevance is the cartographic 
scale at which the metric is calculated, where one must assume that microclimates 
...
were marginal and were associated with high concurvity values among topographic 
...
findings \citep{weiskittel2011forest}. Clearly there are tradeoffs associated 
...
Additionally, in contrast to BAL in this study (displayed in Fig. 
\ref{fig:basesmooths}), others have identified an overall decreasing growth 
effect of BAL, represented by a higher rate of decreases at low values,followed 
by a tapered rate of decrease \citep{Monserud1996,Hann2003,Vospernik2021}. 
In the model presented in section \ref{basemodel}, BAL appears to have a 
...
is a very large presence of them.
\cite{Vospernik2021} found that only including BAL as a competition measure 
...
competitive effects (e.g., BAL) in addition to community crowding effects 
(e.g., BAH). Additionally, \cite{Contreras2011} identified BAL to have a closer 
relationship with growth in larch than other distance-independent measures, 
partially motivating its implementation here. The results in this study 
indicate that including both tree- and stand- level competition 
indices is pertinent in the western larch growth 
relationship, which is consistent with \cite{Wykoff1990}, with the exception 
that this study implemented BAL as a ratio of plot BAH, similar to 
\cite{Stage1976b}(see \cite{Kiernan2008} for comparisons of BAL measures). 
...
however, one must ensure that a smooth has enough space to capture variability 
(or wiggliness) but does not capture \emph{too much} variability. 
The former requires setting the upper limit of a smooth's EDF 
(or basis size; k), and the latter requires that a smoothing parameter is 
estimated \citep[][see section \ref{gamoverview} for more details]{wood2006generalized,Pedersen2019}. 
In this study the choice of k was 10 for DBH , BAL, and BAH 
(10 is the default value provided by mgcv), which seemed appropriate given 
how each function compared to previous studies (as discussed above). 
However, the variability shown at high values of BAH (> 50 $\mathrm{m^2/ha}$) 
in Figure \ref{fig:basesmooths} highlight an instance where 
smoothing complexity could complicate interpretation. 
Although the BAH smooth was fit with high confidence in a non-zero effect 
(p < 0.001; \ref{tab:rmse}), it also had a relatively high complexity 
(> 8 EDF; \ref{tab:rmse}), given the basis size. 
...
effect, and we already knew that growth tends to be limited in 
densely packed stands \citep{Reineke1933}. 
However, in other cases, such as the shade intolerance smooth in 
Figure \ref{ccf_pur_shd}, further constraints may be necessary for more 
accurate characterization and clearer interpretation. 
An additional consideration is the range of data informing each 
...
Given this, the adequacy of basis dimension choice 
...
in productivity may be viewed by the interaction of resource availability, 
...
Mutualism and competition among tree species have been used to explain 
forest development \citep{Oliver}
...
similar or different. Since western larch is known for its high growth 
efficiency and shade-intolerance \citep{Schmidt1976}, it was posited that 
limitations in light acquisition in mixtures with 
...
Whether increasing competition as a result of mixing larch and lodgepole pine 
related to the growth of western larch was unclear in this study. There was,
however, a difference in the way that western larch growth was characterized 
...
The DBH difference smooth shown in Figure \ref{fig:diffsmooths} displays a 
positive difference at values greater than 12 cm, meaning that larch in mixtures 
with lodgepole pine, given this data set, would have higher expected growth at those 
...
Chapter \ref{introduction}. There may be some truth to the positive difference 
...
level (see Table \ref{tab:mixsummary}). Additionally, a visual comparison 
between the pure larch and larch-lodgepole mixtures 
in Figure \ref{fig:sumfig} (first and second rows, respectively) shows few observations of DBH greater 
than 20 cm in the second row, where the slope of the trend lines in the second row 
fail to achieve the same taper as those of the first row at values above 20 cm. 
Therefore, the positive difference at values higher than 20 cm in 
\ref{fig:diffsmooths} may not be subject to valid interpretation. However, 
the negative difference at low DBH values is in concurrence with the stated 
expectation, suggesting that smaller-diameter western larch trees mixed with 
lodgepole pine experience lower rates of growth than the same size trees 
growing primarily with other western larch. Considering the similar shade-tolerances 
...
values of BAL. Since BAL was calculated as plot basal area in trees larger than a subject  
tree divided by total plot basal area, as opposed to a quantity of the former, 
an issue of scale was created when comparing across mixtures in these data. 
For example, if comparing a BAL value of 0.25 
in a plot where the BAH is 40 $m^2/ha$ in a pure larch plot, to a plot where the 
BAH is 10 $m^2/ha$ in a lodgepole pine mixture, the trees of interest are being 
out-competed by 10 $m^2/ha$ and 2.5 $m^2/ha$ worth of are occupied by tree stems, 
respectively. Thus, in this example the relative social position, being 
numerically equal, does not imply the same theoretical quantity of competition 
in larger trees. Considering the relative ranges of DBH values for both lodgepole 
pine mixtures and pure larch mixtures in Table \ref{tab:mixsummary}, where the 
lodgepole pine data are of smaller trees (based on average DBH) than those of 
the pure data, this issue arises. Therefore a competitively dominant larch in a lodgepole 
mixture is likely smaller than one growing with other larch, thus noting a 
negative difference in growth for such a tree is intuitive, given the BAI-DBH 
relationship identified in Figure \ref{fig:basesmooths}. This scaling issue is a 
known weakness of accounting for BAL as a proportion of plot basal area, 
but studies of alternative forms provide mixed results 
\citep{weiskittel2011forest}. Since this model uses a log-link function and 
thus implies a multiplicative effect of variables, BAL is a useful measure of 
social competitive position. However, BAL is confounded with tree size in this 
comparison across conditions, making it difficult to interpret. 



%%%%%%%%%%%%%%%%%%%%%%%%%% Return to this point 82522
