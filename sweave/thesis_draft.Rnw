...
Western larch naturally grows across a gradient in of species-mixtures 
in mixed-species forests \citep{Schmidt1988}. It is sometimes managed 
in even-aged, homogeneous plantations, but more typically is managed in mixed-conifer settings
where interactions among species attributes that impact
growth and development at tree and stand levels may occur.
...
FVS applies a distance-independent individual tree growth model \citep(Stage1973,Wykoff1990) 
to measured tree, density, competition, and site variables to project growth. 
...
Given that individual tree growth models can be improved 
...
The objectives of the study then are to identify a 
distance-independent individual tree growth model using a data set 
which holds data for varying conditions and mixtures, identify different 
types of mixtures that western larch grows in where its growth may be impacted, 
then determine if western larch growth varies depending 
on mixture type after accounting for other factors, and finally
identify the relative importance of species-mixing in estimating growth, 
while comparing different measures of species-mixing to determine if 
one is better at capturing an effect than others. 
...
freedom to provide an unbiased estimate. Explained deviance (CITE??) was also 
...
After establishing a parsimonious base model structure, 
subsets of the PGP data for different types of species-mixtures were determined 
and fit using the same model formulation. Using each resulting model, 
...
species-mix type were then plotted and visually examined. Additionally, 
...
Mixture-types were identified by  
proportions of plot basal area occupied by larch and species it was mixed with. 
For instance, to qualify as a specific mixture-type, a minimum of 20\% total plot 
basal area had to be in western larch trees and a minimum of 20\% plot basal 
area in the other species, where neither larch nor the other species percent 
BA could exceed 65\% in a plot, and both species combined basal area had 
to be at least 70\% of total plot BA. This ensured that a range of 
each mixture-type was captured while prohibiting over-representation of 
either species within the mixture. Additionally, pure larch  
conditions were identified by selecting plots where basal area was composed of 
at least 90\% western larch. Since the data was subset in favor 
...
a stand to express overall competition for crown space. Theoretically, 
different tree species of a given diameter with high maximum crown widths mixed together 
would drive the CCF value well over 100\% representing high competition.  
In contrary, a mixture of species of a given size composed of large and 
small crown widths could increase utilization of crown space, 
reducing competition. This measure does not explicitly  
represent different species proportions, so interpreting species-mixing 
effects from it can be challenging. 
...
In \ref{eq:ccf}, $CCF_{jt}$ is plot $j$ crown competition factor calculated at 
...
In \ref{eq:purity} $L_{jt}$ denotes the ratio, $BA_{ijt}$ is the of basal 
...
purity metric (\ref{tab:shade}). Shade tolerance values were multiplied by individual tree 
basal area values, and then summed, effectively describing community 
shade-intolerance through a proportion of basal area. 
...
In \ref{eq:shade} $T_{jt}$ represents plot shade intolerance at a given measurement, 
...
To evaluate and compare these measures they were added as predictors 
...
accuracy (RMSE) and fit (explained deviance; \citep{wood2006generalized}), quantifying 
each predictor's relative improvement to model performance. 
% To calculate 
...
Random effects were accounted for using a spline basis \citep{Wood2017}, and thus 
had an associated degrees of freedom of ~1338. Model residual 
