...
- using generalized additive models for each task in this study presented 
challenges. One such challenge was the time that it took to fit models 
with random effects. 
- Difficult to interpret, can appear much simpler 
- can overfit relationships
- One benefit was the realized 
- The p-values reported for smooth term significance are uncertain because 
 there is uncertainty in both model selection as well as in smoothing parameter estimation
...
- 
-- BAL EFFECTS ARE IN OPPOSITION TO THE REFERENCE CURVE
 -- this is very interesting. The difference may outweight the curve and in 
 wl-lp conditions, the effect may be inverted. Find out if anyone else has observed this type of effect.
