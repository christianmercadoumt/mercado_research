...
\centering
\includegraphics[width=\textwidth]{../17.figures_saved/base_smooths.png}
\caption{Smoothed effects for pure larch stands fitted to the smooth differences model. The top 4 plots show the 1 dimensional smooth effects for diameter at breast height (DBH), tree crown ratio, ratio of basal area larger than a subject (BAL), and plot basal area (BA). Effect scales vary by smooth.}
\end{figure}
