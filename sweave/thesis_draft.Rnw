...
annual growth of western larch, quantified by basal area increment (BAI; \ref{eq:bai}). 
...
finalizing the base model formulation (\ref{eq:baiform}). 
...
Root mean squared error (RMSE; \ref{eq:rmse}) was 
...
parameters used by the FVS Inland Empire regional variant (\ref{tb:ccfvals};\cite{Keyser2008}).
...
where \ref{eq:shade} $T_{jt}$ represents plot shade intolerance at a given measurement, 
...
To evaluate and compare these measures they were added as additional smooths 
to the identified base model formulation, each in its own respective model, 
and fit with the same data. Models were then compared on the basis of 
...
transformations were applied as in previous works (CITE:stage). Aspect was best 
...
\begin{table}[!h]
\label{tab:rmse}
\caption{Table of effective degrees of freedom (EDF) for each variable, displayed by group. Deviance explained and RMSE values displayed reflect their respective values at the time of model selection, e.g., size, size + competition, etc.}
\centering
...
\ref{tab:mixsummary}. The reference curve captured variability in each 
...
The growth difference in response to crown ratio was slightly elevated 
for larch-Douglas-fir mixtures for mid-range values of CR, but the slight change 
likely doesn't have any important interpretation due to the small impact. 



Growth was not  as responsive in larch-Douglas-fir mixtures,
...
\caption{Smoothed differences for Larch-lodgepole pine mixtures and Larch-Douglas-fir mixtures. Smooths plotted are for only those where that are evidently different, as shown in table 1.4.}
\end{figure}
...
-Plot BA and CCF were highly correlated with one 
another, so CCF was included in place of BA in the model formulation. 
Larch purity and shade intolerance were simply added as an additional 
...
\begin{tabular}{lrrrr}
\toprule
model & rmse\_values & rmse\_values.df & dev.expl & r.sqr\\
\midrule
\cellcolor{gray!6}{Base} & \cellcolor{gray!6}{5} & \cellcolor{gray!6}{3.32} & \cellcolor{gray!6}{84.91255} & \cellcolor{gray!6}{0.8}\\
CCF & 5 & 3.34 & 84.41606 & 0.8\\
\cellcolor{gray!6}{Purity} & \cellcolor{gray!6}{5} & \cellcolor{gray!6}{3.32} & \cellcolor{gray!6}{85.06852} & \cellcolor{gray!6}{0.8}\\
Shade intol. & 5 & 3.32 & 85.01691 & 0.8\\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{BAI model selection}
...
- The p-values are not strictly valid
- We did not explicitly select model degrees of freedom, so that's in our favor
- fitting the model unpenalized could ameliorate issues, but this may render the test pointless
- 
