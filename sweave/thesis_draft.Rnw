...
The model was fit with a gamma distribution because BAI is bounded by zero  and log-link. 
The model training data set consisted of 
...
that represent the overall competitive environment. High-collinear variable 
...
Given the order in which variables were included, 
competition and density metrics contributed the most in deviance explained, 
followed by size. However, considering variables individually, 
DBH explained more deviance and reduced RMSE more than any other 
variable in the model. These results are more or less concurrent with 
