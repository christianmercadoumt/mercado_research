...
The resulting data were pooled together, where each species mixture condition 
was identified and assigned to an identifying variable. These data were then used 
to fit an additive model to the base model formulation, 
however, each mixture was estimated with a unique intercept, and each 
smooth term was evaluated at each type of mixture, resulting in a model 
containing smooths for each species-mixture. Each mixture was evaluated 
against the pure larch condition, where an associated hypothesis test 
was conducted to identify evidence for a difference between each mixture 
and the pure larch condition for each smooth. 
