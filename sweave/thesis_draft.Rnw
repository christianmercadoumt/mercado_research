...
Tree shade tolerance relates to the physiological and morphological characteristics which drive growth \citep{Reich1998}, 
and in-turn, relates to inter-tree competitive dynamics \citep{Canham2006}. Productivity in forests can 
be enhanced by mixing shade tolerant and intolerant species \citep{Williams2017}, 
where different species' niches are considered complementary \citep{Thurm2016}. 
Searle and Chen \citep{-Searle2020} found that positive complementary effects on 
individual tree growth increase with community shade-tolerance dissimilarity.
...
competition for light can sufficiently capture the effects of biodiversity on productivity \citep{Morin2011}. 
Given this information then, a shade tolerance informed metric may capture complementarity 
...
\emph{How does community composition (species-mixing) influence the growth of western larch?} 
...
\emph{Comparing species-informed measures, how can the effects of species-mixing be best accounted for within an individual-tree growth model?}

Therefore, my related research objectives were:
%(i) to identify variables that capture the effect of species-mixing on growth 
(i) determine if western larch growth varies depending on what type of mixture it is in 
