...
fit, a random effect was specified at the individual tree-level.

---include something visual about dependence structure?---

GAMMs were fit using using the mgcv package \citep{Wood2011} in R \citep{RCoreTeam2021}. 
...
constrains a smoother's complexity as it grows higher \citaep{Pedersen2019}. 
...
better comparison across models that use different terms. 
...
when fitting large numbers intercepts (as done in this study) 
...
\text{$g()$ represents a link function, $E(BAI)$ represents the expected 
...
freedom to reduce bias. Explained deviance (CITE??) and AIC (CITE?) were also 
considered for ensuring acceptable model fit, %Do i even say this?
but they were not used as strict selection criteria because
each improved proportionally to RMSE. %Do i even say this?
...
either species within the mixture. Since the data was subset in favor 
of these specific scenarios, it was important to consider the size of subsets 
before fitting models with them. Ultimately, mixtures chosen were composed of 
...
of BAI across the range of DBH. However, since the values of 
...
area occupied by western larch. 
...
\text{$L_{jt}$ denotes the ratio, $BA_{ijt}$ is the of basal area of a tree $i$, $I_{ijt}$ is a binary operator which indicates whether a tree $i$ is western larch ($I_{ijt} = 1$), or otherwise ($I_{ijt} = 0$)}
...
purity metric (REFERENCE SHADE INTOLERANCE TABLE). Shade tolerance values were multiplied by individual tree 
...
\text{$\rho_{ijt}$ represents the species specific shade-tolerance ranking of tree $i$ \citep{Lienard2015}.}
\end{equation}
...
accuracy (RMSE) and fit (explained deviance; CITE?), quantifying 
each predictor's relative improvement to model performance. To calculate 
