...
Western larch naturally grows across a species-compositional gradient in mixed-species
forests \citep{Schmidt1988}. It is sometimes managed 
...
Stand density %, pertaining to competition-dependent mortality, or self-thinning (CITE),
can be modified at different levels of species 
...
Altering density can modify competition, 
which has further implications on stand productivity and tree growth.
...
and was attributed to the shade tolerance of each species \citep{Maguire2021}. 

In fact,
Brunner \& Forrester (\citeyear{Brunner2020}) found that mixture 
...
light requirement (or shade tolerance) niches are complementary \citep{Thurm2016}. 
...
At a finer scale, Searle and Chen (\citeyear{Searle2020}) found that 
positive complementary effects on individual tree growth increase with both community 
shade-tolerance and phyologenetic dissimilarity.
...
\begin{enumerate}
\item \bf{How does community composition (species-mixing) influence the growth of western larch?} 
%alternative: How does species-mixing impact basal area increment of western larch?
\item \bf{Comparing species-informed measures, how can the effects of species-mixing be best accounted for within an individual-tree growth model?}
\end{enumerate}
