...
\citep(Stage1973a,Wykoff1990) that does not require individual tree location 
...
\begin{table}[H]
\caption{Counts of data for each stand. The first 2 digits of each stand number correspond to which national forest they are in: 14 for the Kootenai NF, and 16 for the lolo national forest. }
\label{tab:sumtab}
\centering
\begin{tabular}{lrrrr}
\toprule
