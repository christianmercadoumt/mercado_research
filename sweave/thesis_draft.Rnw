...
--TABLE OF \#of PLOTS, \# OF TREES, Range of conditions--
...
annual growth of western larch quantified by basal area increment (BAI) -EQUATION for BAI-. 
GAMMs allow for flexibility in model fitting and an allowance for non-linear 
...
-discuss thin plate smooths, allowed mgcv model fitting to select smoothness/penalties/etc-?
...
The resulting formulation was used to evaluate effects of predictors 
...
based on the following groups in order of how much variation in BAI they captured: 
size, density \& competition, and site. 
Variable selection was performed group-by-group in an additive manner. 
For example, size variables were compared first, then, additive to the selected 
size variable(s), competition and density variables were compared, 
followed by site variables.

-BAI MODEL HERE-

GAMMs were used to select variables and examine 
predictor effects on the resulting modeled relationship. 
Variables were added to the model respective of their groupings, 
and highly correlated same-group variables were compared in alternative models.
Root mean squared error (RMSE; \ref{eq:rmse}) was 
calculated to assess model accuracy, where when comparing models, 
a model with the lowest RMSE (highest predictive accuracy) was selected. 
RMSE was calculated using residual degrees of freedom to remove bias. 
Explained deviance (CITE??) and AIC (CITE?) were also considered for evaluating model fit, %Do i even say this?
but not used as strict selection criteria. %Do i even say this?
...
due to hierarchical groupings in the sample-design over inconsistent remeasurement periods
% , and 
