...
and fit using the same model formulation. Using each species-mixture model, 
growth was predicted from a data set holding all but one variable constant 
to isolate the effects of that variable, and then was plotted to examine  
effects. Additionally, to investigate whether the given variable relationship 
with BAI was impacted at different levels of another variable, the same 
