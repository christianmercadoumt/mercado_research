...
that is exclusive to the inland northwest region of North America. It is 
very intolerant of shade and is managed across a range of communities with 
both shade-tolerant and -intolerant species. Various recent works have 
shown that tree and stand level competition and density in mixed-species 
...
larch after accounting for other factors that influence growth and (ii) using 
readily available information, how to characterize species-mixing effects on 
...
growth from size, competition, density, and site productivity variables, but that 
did not address species-mixing. This model was then used in two separate analyses. 
In the first analysis, we identified different types of species mixtures and 
compared how the established western larch growth relationship in each mixture 
differed from that of non-mixed pure larch stand conditions. In the second 
...
and site variables with functional relationships that are similar to those 
...
However, data and design limitations prevent further extrapolation for the 
mixture-pure comparisons. When comparing species-mixing metrics, we showed 
that western larch growth may be positively impacted in mixtures with species 
of higher shade-tolerance than larch as well as in mixtures with other species 
in general. 
