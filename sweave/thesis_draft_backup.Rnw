...
commercially available in a mix with Douglas-fir lumber, adding a unique and
...
fire resistant and a prolific pioneer species on sites following fire \citep{Schmidt1981}
...
Western larch naturally grows across a gradient of species-mixtures 
in mixed-species forests \citep{Schmidt1988}. It is found in communities with:
Douglas-fir (\emph{Pseudotsuga menziesii} var.\emph{glauca}), 
lodgepole pine (\emph{Pinus contorta}), Engelmann spruce (\emph{Picea engellmanii}),
...
typically is managed in mixed-conifer settings where interactions among species 
...
its communities \citep{Baker1949,Schmidt1988}. 
Larch's prolific regeneration and rapid juvenile growth allows it to 
out-compete other trees, which can lead to high stand density \citep{Schmidt1998}.  
...
maximum stand density increased in shade intolerant \emph{Pinus ponderosa} and 
\emph{Pseudotsuga menziesii} when species-diversity was higher, but in 
shade-tolerant \emph{Abis grandis}, the relative impact 
of stand purity on stand density was unimportant \citep{Kimsey2019}. 

Density modifies inter-tree competition and tree growth, 
...
Concurrently, in a study of \emph{Abis grandis} and \emph{Pinus ponderosa} mixtures in Oregon,
Maguire and Mainwaring (\citeyear{Maguire2021}) found that 
mixing impacted tree size only at high density at one study site. 
At the other site, however, the number of observations suggesting no effect on growth 
...
capture the effects of biodiversity on productivity \citep{Morin2011}, given 
...
tree growth increase with both community shade-tolerance and phyologenetic 
dissimilarity, showing that shade-tolerance can encapsulate its effects on 
inter-specific competition, and that positive niche complementary 
...
to measure competition, and projects tree growth using tree size, stand density, 
...
growth \citep{weiskittel2011forest}. 
Various studies have evaluated species-mixing effects on tree growth
using the solid framework of such models. 
...
photogenically similar European larch, then species-mixing effects may be relevant 
for characterizing western larch growth because it is a fast growing and 
...
(ii) how to characterize species-mixing effects 
...
The objectives of the present study then were: 
(i) to identify a 
distance-independent individual tree basal area increment (BAI) model to 
statistically control for variations in tree size, stand density, 
...
(ii) to determine if western larch BAI varies by mixture type after 
accounting for other factors, and
(iii) to compare generalizable species-mixing measures and identify 
which of these improves growth estimation. 
I expected that larch growth would be positively impacted 
...
Additionally, I expected that species-mixing effects would best be captured by 
a measure that accounts for different species' shade tolerance, as this would 
address the primary mechanisms behind variations in growth, as opposed to 
measures of stand species purity or crown density. 
...
tree plots, counts by height and species were recorded for trees greater 
...
damage to the tree and its intensity. For small trees taller than 15 cm but 
shorter than 1.4 m, species, live/dead status, crown ratio, height, and crown class were 
recorded. For small trees tall enough for DBH measurement and with a DBH below 
...
1.4 m to 3.7 m, 3.7 m to 5.8 m, and trees taller than 5.8 m (tallied individually). 
...
\begin{equation}
\label{eq:bai}
BAI_m = \frac{BA_{m+1}-BA_{m}}{t_{m+1}-t_m}
\end{equation}

, where $BA_m$ refers to tree basal area at measurement $m$ and $t_m$ to the 
measurement occasion and $m+1$ is a subsequent measurement occasion.
...
stand dynamics over time \citep{weiskittel2011forest}.
...
Size variables include $DBH$, and tree basal area ($BA$). 

Calculated density and competition variables were: plot number of trees per hectare 
($TPH_{pm}$), plot basal area per hectare ($BAH_{pm}$), 
ratio of plot BAH in trees that are larger than a focal tree of interest 
($BAL_{km}$), plot quadratic mean diameter ($QMD_{pm}$), ratio of tree DBH to QMD ($Dq_{km}$)

%ADD SITE HERE

In the following definitions, $k$ denotes an individual tree, $p$ represents a 
unique plot, $m$ represents a measurement occasion, each is omitted. 

\begin{equation}
\begin{aligned}
BA_{km} &= \pi(\frac{DBH_{km}}{2})^2\\
TPH_{km} &= hCount_{km}\\
TPH_{pm} &= \sum_p TPH_{km}\\
BAH_{pm} &= \sum_p hBA_{km}\\
BAL_{km} &= \frac{BA_{km}}{\sum_{p, BA_{km} > BA_{km}} BA_{km}}\\
QMD_{pm} &= \frac{BAH_{pm}}{0.0000785TPH_{pm}}\\
Dq_{km} &= \frac{DBH_{km}}{QMD_{pm}}\\
\end{aligned}
\end{equation}
% 0.0000785
...
% \begin{table}[!h]
% \caption[Summary of predictors for each stand]{A summary of predictors mean and sd}
% \label{tab:sumpre}
% \centering
% \begin{tabular*}{\textwidth}{llrrrrrrrr}
% \toprule
% Stand & Aspect & Mean DBH & SD DBH & Mean CR & SD CR & Mean BAL & SD BAL & Mean BAH & SD BAH \\
% \midrule
% \cellcolor{gray!6}{1401} & \cellcolor{gray!6}{ENE} & \cellcolor{gray!6}{43.412} & \cellcolor{gray!6}{7.02} & \cellcolor{gray!6}{0.34} & \cellcolor{gray!6}{0.08} & \cellcolor{gray!6}{0.42} & \cellcolor{gray!6}{0.29} & \cellcolor{gray!6}{47} & \cellcolor{gray!6}{11}\\
% 1402 & SE & 7.649 & 3.60 & 0.85 & 0.10 & 0.64 & 0.26 & 7 & 5\\
% \cellcolor{gray!6}{1403} & \cellcolor{gray!6}{WNW} & \cellcolor{gray!6}{8.264} & \cellcolor{gray!6}{4.92} & \cellcolor{gray!6}{0.88} & \cellcolor{gray!6}{0.10} & \cellcolor{gray!6}{0.64} & \cellcolor{gray!6}{0.32} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{5}\\
% 1404 & WNW & 10.073 & 4.95 & 0.84 & 0.13 & 0.61 & 0.29 & 12 & 9\\
% \cellcolor{gray!6}{1405} & \cellcolor{gray!6}{SW} & \cellcolor{gray!6}{11.528} & \cellcolor{gray!6}{3.31} & \cellcolor{gray!6}{0.41} & \cellcolor{gray!6}{0.13} & \cellcolor{gray!6}{0.57} & \cellcolor{gray!6}{0.27} & \cellcolor{gray!6}{17} & \cellcolor{gray!6}{7}\\
% \addlinespace
% 1406 & SW & 10.462 & 4.87 & 0.80 & 0.14 & 0.59 & 0.29 & 9 & 6\\
% \cellcolor{gray!6}{1407} & \cellcolor{gray!6}{WNW} & \cellcolor{gray!6}{12.465} & \cellcolor{gray!6}{6.03} & \cellcolor{gray!6}{0.76} & \cellcolor{gray!6}{0.14} & \cellcolor{gray!6}{0.53} & \cellcolor{gray!6}{0.31} & \cellcolor{gray!6}{10} & \cellcolor{gray!6}{7}\\
% 1408 & ENE & 30.641 & 7.92 & 0.32 & 0.09 & 0.52 & 0.30 & 41 & 18\\
% \cellcolor{gray!6}{1609} & \cellcolor{gray!6}{SE} & \cellcolor{gray!6}{10.604} & \cellcolor{gray!6}{5.61} & \cellcolor{gray!6}{0.77} & \cellcolor{gray!6}{0.14} & \cellcolor{gray!6}{0.59} & \cellcolor{gray!6}{0.33} & \cellcolor{gray!6}{9} & \cellcolor{gray!6}{6}\\
% 1610 & SW & 7.963 & 3.98 & 0.77 & 0.13 & 0.65 & 0.29 & 9 & 6\\
% \addlinespace
% \cellcolor{gray!6}{1611} & \cellcolor{gray!6}{ENE} & \cellcolor{gray!6}{9.380} & \cellcolor{gray!6}{4.30} & \cellcolor{gray!6}{0.76} & \cellcolor{gray!6}{0.15} & \cellcolor{gray!6}{0.62} & \cellcolor{gray!6}{0.32} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{7}\\
% 1612 & SW & 8.782 & 5.37 & 0.58 & 0.25 & 0.67 & 0.28 & 10 & 6\\
% \cellcolor{gray!6}{1614} & \cellcolor{gray!6}{NNW} & \cellcolor{gray!6}{14.503} & \cellcolor{gray!6}{6.88} & \cellcolor{gray!6}{0.81} & \cellcolor{gray!6}{0.11} & \cellcolor{gray!6}{0.49} & \cellcolor{gray!6}{0.33} & \cellcolor{gray!6}{9} & \cellcolor{gray!6}{4}\\
% 1615 & WNW & 12.537 & 4.24 & 0.84 & 0.11 & 0.53 & 0.30 & 12 & 7\\
% \cellcolor{gray!6}{1616} & \cellcolor{gray!6}{WNW} & \cellcolor{gray!6}{16.932} & \cellcolor{gray!6}{4.89} & \cellcolor{gray!6}{0.40} & \cellcolor{gray!6}{0.11} & \cellcolor{gray!6}{0.52} & \cellcolor{gray!6}{0.29} & \cellcolor{gray!6}{30} & \cellcolor{gray!6}{11}\\
% \addlinespace
% 1617 & SE & 9.247 & 2.72 & 0.79 & 0.12 & 0.59 & 0.26 & 8 & 3\\
% \cellcolor{gray!6}{1618} & \cellcolor{gray!6}{ENE} & \cellcolor{gray!6}{8.247} & \cellcolor{gray!6}{4.61} & \cellcolor{gray!6}{0.74} & \cellcolor{gray!6}{0.14} & \cellcolor{gray!6}{0.68} & \cellcolor{gray!6}{0.29} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{6}\\
% 1619 & NNE & 12.365 & 4.49 & 0.71 & 0.14 & 0.52 & 0.28 & 17 & 9\\
% \bottomrule
% \end{tabular*}
% \end{table}
...
\section{GAM overview}

Using these data, a generalized additive mixed model (GAMM) was developed. 
...
effects with a sum of \emph{smooth} functions of covariates instead of 
...
The mgcv package allows for the specification of smooth functions (or smoothers). 
Smoothers are a sum of fitted estimated basis 
functions that characterize the conditional relationship between the predictor 
and independent variable. They are penalized by a smoothing parameter, which 
constrains a smoother as its complexity grows higher \citep{Pedersen2019}. 
...
The maximum likelihood method was used in this study because it allows for 
likelihood-based comparison across models with different fixed terms and smoothing parameters. 
Random effects may also be coded as penalized smooths in this format \citep{Pedersen2019}. 
% , however, 
...
With mgcv, the thin-plate regression spline (TPRS) type
is the default owing to its simple and effective behavior \citep{Wood2003},
and was used for all terms. 
% GAMs in mgcv also allow for 


The model formulation described here was developed with a training 
data set which allowed for varying conditions across all variables. 
Data for two measurement plots out of each stand were withheld for 
the purpose of model validation and assessment.
The model formulation resulting from the selection process described below
was used to evaluate effects of predictors 
on BAI across the entire data set. The formula was also used to assess smooths
across different species mixtures, where the model was re-fit with a subset 
of the training data set where species mixture type was categorized.  
...
Predictor selection followed the logic of \cite{Wykoff1990}, 
(\cite{Monserud1996}), and \cite{Vospernik2021} where predictors were 
grouped together based on the following categories: 
(i) size, (ii) density and competition, and (iii) site predictors. 
Size variables considered were tree diameter at breast 
height ($DBH$) and tree basal area ($BA$). 
Density and competition variables considered were: plot basal area ($BAH$), 
plot basal area larger than the subject tree (BAL; expressed as a ratio of BAH), 
plot quadratic mean diameter ($QMD$), ratio of tree DBH to QMD ($D_q$), 
and trees per hectare ($TPH$). Site variables initially considered were: 
slope, aspect, elevation, mean site index, and heatload \citep{Theobald2015}. 
These initial explanatory variables were identified based on availability 
in the PGP data set, whether they could be calculated from the 
available data, and if they have been used in predicting 
...
A pair-wise colinearity assessment was 
then performed within each group of variables to avoid including 
highly correlated variables together in the model. 
Any predictors with a pair-wise correlation of 
0.7 or higher were not considered in a model together. Although somewhat arbitrary, 
this threshold was selected because a 0.7 correlation indicates that  
variables are closely related. Additionally, variables 
known to represent the same processes, even if not highly linearly correlated 
(such as heatload and aspect, for instance), were not considered together to 
avoid redundancies and reduce model complexity. 

GAMs were used to select predictors and visually examine 
predictor partial effects on BAI. % as well as examine residual structure. 
Models were fit in iterations of predictor grouping. 
% including non-correlated variables together 
% respective of grouping while maximizing model accuracy and avoiding overlap. 
The order of this group-wise selection process was based on 
previously identified contributions of these predictor groups 
in estimating BAI \citep{Wykoff1990,Monserud1996}. Highly 
correlated same-group variables were compared in alternative models.
...
The same process was used for site variables thereafter, 
finalizing the base model formulation: 

\begin{equation}
\label{eq:baiform}
ln[E(BAI)] = \beta_0 + \sum_j f_{1j}(x_{1j}) + \sum_j f_{2j}(x_{2j}) + \sum_j f_{3j}(x_{3j})\\
\end{equation}

where $ln$ represents the logarithmic link function, $E(BAI)$ represents the expected 
value of BAI, $\beta_0$ represents an intercept, $\sum_j$ represents summation 
of smooth functions for a given group, $x_1$, $x_2$, $x_3$ represent 
size, competition, and site predictor variables, respectively, and $f_1$, $f_2$, $f_3$ 
are smooth functions for each, respectively. 

Root mean squared error (RMSE; Eq. ~\ref{eq:rmse}) was calculated to assess 
model accuracy: 

\begin{equation}
\label{eq:rmse}
RMSE = \sqrt{\frac{\sum_k (\widehat{BAI_k} - BAI_k)^2}{n_{df}}}
\end{equation}

where $\widehat{BAI_k}$ is the model prediction, 
$BAI_k$ is the observed value, and $n_{df}$ is the model residual 
degrees of freedom (number of increments less model effective degrees of freedom).

When comparing models, a model formulation with the lowest RMSE (highest predictive accuracy) 
was selected over alternatives. RMSE was calculated using residual degrees of 
freedom to provide an unbiased estimate. The proportion of null deviance 
explained by the model (or deviance explained) \citep{wood2006generalized} was also 
considered for ensuring acceptable model fit, %Do i even say this?
but was not used as a primary selection criterion. Additionally, 
A concurvity \citep{Hastie1990} assessment was also conducted between variables 
within each group at each phase of group variable selection. 
Concurvity is an evaluation of whether one smooth function in a model describes 
a different covariate smooth function in the same model 
\citep{wood2006generalized}. If estimated concurvity was greater than 0.8 for a particular 
set of terms, then an alternative not including both terms was evaluated.
...
species mixtures, distinct mixtures were determined based on species 
proportional share of plot basal area.%BAH. 
...
in either Douglas-fir or lodgepole pine. This was to ensure that there was a minimal 
representation by both species. To ensure that neither species was  
over-represented in a plot, neither larch nor the other species 
could exceed 65\% of plot BA. Also, both species combined basal area had 
to occupy at least 70\% of total plot BA. 
...
A pure larch condition was defined as at least 70\% of plot BA in western larch. 

The mgcv package allows for smooth functions that vary across different 
groups \citep{Pedersen2019}. A variation of this method was employed to 
contrast western larch growth across each identified type of mixture against 
the pure larch stand condition. Thus, the identified 
base model formula was fit but where each mixture was allowed an offset for the intercept
and for each smooth term, resulting in a model containing a reference smooth 
for each predictor, and a \citep[\emph{difference smooth}][]{wood2006generalized} for species-mixture: 

\begin{equation}
\label{eq:mixes}
\begin{aligned}
ln[E(BAI)] =& \beta_0 + \sum_j f_{j}(x_{j}) + \sum_s \delta_s(\alpha_s + \sum_j g_{j,s}x_j) \\
\delta_s =& 
\begin{cases}
0 & \text{if pure larch}\\
...
to identify evidence that a difference is non-zero. 
...
crown competition factor, proportion of basal area occupied by larch, and 
proportion of basal area weighted by species-shade tolerance values. 

Crown competition factor (CCF) is a species-informed density measure that 
...
a given DBH. Crown width is associated with a tree's maximum crown area 
(MCA), and can be estimated as a proportion of stand area using species-specific 
coefficients in a polynomial equation.  
These maximum crown area values are summed within a stand to express 
overall competition for crown space, or CCF:

\begin{equation}
\label{eq:ccf}
CCF = \sum_k w_k h(DBH_k, s_k)
\end{equation}

where $w_k$ is the area expansion factor (TPH) for tree $k$, and $h()$ is a 
function of the DBH of tree $k$, given its species $s$. This study used the 
species-specific coefficient values identified by the FVS-IE variant \citep{Keyser2008}.
This data is normalized such that a CCF value of 100 means that all trees 
in a stand have theoretically achieved their maximum crown area and 
canopy closure is achieved, i.e., any value in excess of 100 
theoretically describes the threshold of competition for crown space. 
...
ratio of plot basal area occupied by western larch ($BA_{s_km}$) to total plot 
basal area $BAH_m$: 

\begin{equation}
\label{eq:purity}
L = \frac{\sum\limits_{k} BA_{s_k}}{\sum\limits_{k} BA_{k}}
\end{equation}

where $L$ denotes the plot larch purity value at measurement $m$, for species $s$ 
and $BA_{s_k}$ is the basal area of tree $k$, given its species identity, which 
is western larch in this case. 

%  \frac{\sum_{i}{BA_{ijt}\ I_{ijt}}}{\sum_{i}{BA_{ijt}}}\ \ \ I_{ijt} = \begin{cases} 1,\ \text{if}\ i\ \text{is western larch}\\0,\ \text{otherwise}\end{cases}
% \end{equation}
...
% \end{equation}
% 
% where $L_{jt}$ denotes the ratio, $BA_{ijt}$ is the of basal 
...
purity metric. The resulting values are shown in Table ~\ref{tab:shade}. 
Shade intolerance values were multiplied by individual tree 
basal area values, and then summed: 

\begin{equation}
\label{eq:shade}
T = \frac{\sum\limits_{k}{\rho_{s} BA_{s_k}}}{\sum\limits_{k} BA_k}
\end{equation}

where $T$ represents plot shade intolerance at a given measurement and 
$\rho_{s}$ represents the species $s$ shade intolerance value for  
tree $k$ \citep{Lienard2015}. $T$ effectively describes community 
shade-intolerance through a proportion of plot basal area, where a $T$ value of 1 
...
\begin{table}
\caption[Shade intolerance values]{Shade intolerance values, which are the inverse of shade tolerance values provided by Lienard (\citeyear{Lienard2015})}
\label{tab:shade}
\centering
...
accuracy (RMSE) and fit \citep[deviance explained;][]{wood2006generalized}, quantifying 
each predictor's relative improvement to model performance. RMSE values were 
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{../17.figures_saved/bai_diam_sumfig.png}
\caption[BAI-Diameter by BAH and aspect]{Individual tree BAI and DBH, gridded across aspect and BAH. Aspects are coded by cardinal direction, where N,S,E,W are north, south, east, and west, repsectively (e.g., NNW = north-northwest). Thin grey lines indicate 0 on each axis, with a 1:1 slope dotted-line passing through (0,0) included for visual reference. There were no observations in SW aspects, and all east-facing southerly aspects were binned together, as there was very little data in that quadrant.}
\label{fig:sumfig}
\end{figure}

%begin with description of the raw data

In the PGP data, tree size was positively associated with the growth of western larch 
and this relationship varies across the range of aspect and density, 
as shown in figure \ref{fig:sumfig}. Growth appears the most vigorous 
in trees less than 20 cm in DBH at low densities, where a nonlinear growth 
relationship is apparent. At moderate to higher densities, however, DBH and 
...
aspects towards northern and eastern aspects, the DBH-BAI trend appears less 
...
The choice of fitting non-linear relationships for predicting tree growth with a gamma 
distribution and a long-link function appears to be 
valid given the dispersion and non-linearity present in these relationships. 
...
Tree DBH and basal area were highly correlated (0.938) and were 
evaluated in separate models. DBH as a predictor yielded slightly higher 
accuracy in predicting BAI than tree BA, with model RMSE values  
of 5.64 $\mathrm{cm^2/yr}$ and 5.70 $\mathrm{cm^2/yr}$, respectively. 

Competition variables were partitioned into two sub-groups:  
variables representing a tree's competitive position in a stand 
(BAL, $\mathrm{D_q}$, and CR), and variables that represent the overall competitive 
environment (QMD, TPH, BAH, and CCF). Highly collinear variable 
combinations were were TPH-$\mathrm{D_q}$ (0.732) and BA-QMD (0.684).  %, CCF-BA (0.95)
With DBH effects included, adding tree-level variables reduced RMSE to 4.50 
$\mathrm{cm^2/yr}$, and then to 4.31 $\mathrm{cm^2/yr}$ with plot BA included. 
CR was applied as a tree-level competition instead of a tree size variable 
because it reflects the cumulative effects of past 
management and competition \citep{Wykoff1990,Monserud1996}, but either way 
it would have been included because in both scenarios it improved model accuracy. 
...
Aspect was best accounted for as an interaction of the sine and cosine terms 
in the form of an isotropic smooth.
...
the best term, further reducing model RMSE to 4.01 $\mathrm{cm^2/yr}$

%Then describe the numbers and introduce the table
By group, competition metrics contributed the most in 
deviance explained, followed by size (Table \ref{tab:rmse}). However, considering 
variables individually, DBH explained more deviance and reduced RMSE 
...
\begin{table}[!h]
\caption[Base model selection criteria]{Table of effective degrees of freedom (EDF) for each variable, displayed by group. Deviance explained and RMSE values displayed reflect their respective values at the time of model selection, e.g., size, size + competition, etc. RMSE for model selection was calculated on the data used to train models in the selection period, but was degrees of freedom as to avoid bias.}
\label{tab:rmse}
\centering
...
\toprule
\hspace{1em}Smooth & EDF & Deviance explained & RMSE ($\mathrm{cm^2/yr}$)\\
\midrule
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{Size}} & 30.82 & 5.64312 \\
\hspace{1em}\cellcolor{gray!6}{DBH} & \cellcolor{gray!6}{8} & \cellcolor{gray!6} & \cellcolor{gray!6} \\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{Competition}} & 62.87 & 4.31 \\
...
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{Site}} & 67.39 & 4.01197\\
\hspace{1em}\cellcolor{gray!6}{Aspect} & \cellcolor{gray!6}{26} & \cellcolor{gray!6} & \cellcolor{gray!6} \\
\addlinespace[0.3em]
\textbf{Base model} & 1389 & 84.91 & 3.32142 \\
\hspace{1em}\cellcolor{gray!6}{Random intercept} & \cellcolor{gray!6}{1337} & \cellcolor{gray!6} & \cellcolor{gray!6} \\
\bottomrule
\end{tabular}
\end{table}

A basal area increment model was specified with the following predictors: 
DBH, CR, BAL, BAH, aspect, and a tree-level random effect. 
The model training data set consisted of %training data set?
5,792 basal area increments across 1,975 individual western larch trees. 
The base model RMSE was ~3.34 $\mathrm{cm^2/yr}$ Fixed effects terms used 
~53 degrees of freedom, and total model deviance explained was 84.9\%. 
Random effects were accounted for using a spline basis 
\citep{wood2006generalized}, and thus had an associated degrees of 
freedom of ~1338. Model residual degrees of freedom was ~4401. 
Model statistics are reported in table \ref{tab:rmse}.

Partial response curves are shown in Figure \ref{fig:basesmooths}. DBH had an 
overall increasing effect on BAI, eventually tapering at values larger 
than 30cm. Crown ratio was penalized to a near-linear increasing effect that 
was relatively small overall, going from negative to positive at values near 
0.7. BAL showed a modest positive effect which decreased to a slightly 
larger negative effect as western larch became less dominant in a plot. 
Basal area (BA) was represented by an overall decreasing effect, where densities greater 
than 10 $\mathrm{m^2}/ha$ were associated with an increasingly negative effect on growth, 
that tapered off and became less certain at values exceeding 40 $m^2/ha$. As 
aspect varied, the partial effect on growth varied, where positive effects 
were associated with N-NW, N-NE, and NE aspects. NW and SE aspects were associated 
with some negative growth. Data was not sufficient for esimating aspect effects 
for south-facing aspects. 

\begin{figure}
\centering
% \begin{subfigure}[b]{0.7\textwidth}
\includegraphics[width=0.7\textwidth]{../17.figures_saved/base_smooths.png}
% \end{subfigure}
% \begin{subfigure}[b]{0.7\textwidth}
% \includegraphics[width=\textwidth]{../17.figures_saved/base_aspect.png}
% \end{subfigure}
\caption[Base model smooths]{Smoothed effects fitted by the selected base model. The top 4 plots show the 1 dimensional smooth effects for diameter at breast height (DBH), tree crown ratio, ratio of basal area larger than a subject (BAL), and plot basal area (BA). Effect scales vary by smooth.}
...
(Table ~\ref{tab:mixsummary}). Pure larch stands evaluated sat on northerly 
aspects spanning from west to east (Fig. ~\ref{fig:asp}), and both mixtures 
evaluated were more abundant on northwest-facing slopes compared to other 
aspects, as shown in figure \ref{fig:aspcnts}. This was a non-issue in terms 
of bias, as ultimately there was data across a similar range of aspect for 
each mixture. Tests indicating certainty of non-zero effects for estimated 
...
A reference smooth function was fit for western larch for each predictor, and 
is shown in figure \ref{fig:diffsmooths}. 
The reference smooth for each 
predictor captured variability with high certainty (p-values $< 0.001$). 
Somewhat unsurprisingly, each reference smooth closely resembled the 
corresponding smooths fitted by the base model. Crown ratio was 
penalized to a linear effect but strong evidence for a non-zero effect remained. 
There was also evidence indicating that the mean value of the response for pure larch 
conditions was not zero. 

%Table of mixtures, min, max, mean for each variable in model

\begin{table}[H]
\caption[Mixtures data summary]{Summary table for different species-mixture data subsets.}
\label{tab:mixsummary}
\centering
\begin{tabular}{llrrr}
\toprule
Variable & & Pure larch & Larch-lodgepole & Larch-Douglas-fir\\
\midrule
\multicolumn{1}{c|}{\cellcolor{gray!6}}& \cellcolor{gray!6}{min} & \cellcolor{gray!6}{0.05} & \cellcolor{gray!6}{0.03} & \cellcolor{gray!6}{0.10}\\
\hhline{~|}
...
\subsection{Western larch-lodgepole pine mixtures}
% Western larch growth varied in mixture type compared to homogeneous conditions 
% in response to each variable. 
There was strong evidence indicating a departure of growth response from pure conditions
as DBH, symmetry of competition BAL, and stand density BA varied
when mixed with lodgepole pine, and modest  
evidence for such in crown ratio and aspect (Table ~\ref{tab:diff}). 
In these mixtures, growth was less in smaller western larch trees, 
but became greater when larch diameters exceeded ~12 cm, compared to pure stands. 
A similarly-shaped difference effect is displayed as BAL increases from zero.  
This means that a western larch tree without larger neighbors (at BAL values 
close to zero) would have lower growth when 
growing in a mixture with lodgepole pine compared to pure-larch 
conditions. Inversely, a larch would achieve higher growth when 
more of its neighbors are larger than it (BAL $> 0.75$ plot BA) when mixed 
with lodgepole pine compared to a larch growing in a pure stand. 
Furthermore, the BAI-BAH difference smooth showed a positive difference at 
low densities and a negative difference at high densities. However, the 
...
\subsection{Western larch-Douglas-fir mixtures}

For larch-Douglas-fir mixtures, growth deviated from the pure stands 
as crown ratio, stand density, and aspect varied, but little to no variation was 
observed in response to different levels of tree size or symmetry of competition. 
The growth difference in response to crown ratio were slightly elevated 
for larch-Douglas-fir mixtures and only in mid-range values of CR (Fig. ~\ref{fig:diffsmooths}). 
Similar to the lodgepole pine mixtures, basal area impacts on 
growth in Douglas-fir mixed larch were were intensified at higher densities. 
...
\begin{table}
\centering
\caption[Difference smooths table]{Table of effective degrees of freedom for difference smooths and p-values. Estimated intercept values for the reference smooth and two conditions are shown in the lower portion, where the reference p-value indicates if the mean bai (response) for pure conditions differs from zero, and if each mixture's average values differ from the mean value of the reference.}
\label{tab:diff}
...
\bottomrule
\multicolumn{3}{l}{Intercept} & \multicolumn{2}{c}{Estimate} & \multicolumn{2}{c}{p-value}\\
\midrule
\multicolumn{3}{l}{\cellcolor{gray!6}{Reference}} & \multicolumn{2}{c}{\cellcolor{gray!6}{-0.0452}} & \multicolumn{2}{c}{\cellcolor{gray!6}{$0.018$}} \\
\multicolumn{3}{l}{Larch-lodgepole} & \multicolumn{2}{c}{-0.256} & \multicolumn{2}{c}{0.694} \\
\multicolumn{3}{l}{\cellcolor{gray!6}{Larch-Douglas-fir}} & \multicolumn{2}{c}{\cellcolor{gray!6}{-0.138}} & \multicolumn{2}{c}{\cellcolor{gray!6}{$0.778$}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{../17.figures_saved/mix_diff_smooths.png}
\caption[Difference smooths]{Smoothed differences for Larch-lodgepole pine mixtures and Larch-Douglas-fir mixtures are shown on the right and reference curves are shown on the left for visual comparison.}
...
Both shade intolerance and purity were fit as additional 
predictors in respective models. CCF and BAH 
were found to be highly correlated with one-another, so ccf was included 
in its respective model in place of BAH, as to avoid issues of collinearty  
and concurvity among predictors. 

%Table w/ rmseand stuff (is below)
Model accuracy was best improved by the addition of stand purity, followed by 
plot shade intolerance and CCF (Table ~\ref{tab:mixvarsrmse}). 
Every term in each respective model was backed by a p-value $< 0.001$, indicating 
that each was confidently non-zero. Changes in RMSE were modest, all of which 
were at the sub-square centimeter per year -level. Model fit increased in concert with 
improvements to accuracy except for with CCF, which 
showed a small decrease in deviance explained (Table ~\ref{tab:mixvarsrmse}) and 
$R^2$.

In the absence of BAH in the model, CCF displayed a larger effect on tree growth 
...
to the BAH smooth in the base model, showing a declining effect from low to high values. 
Both purity and shade intolerance were also characterized by declining  
effects on growth as each increased, but since density is already accounted for 
by BAH in each of these models, both of these measures have an 
expected smaller effect on growth. 
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{../17.figures_saved/ccf_pur_shd.png}
\caption[CCF, purity, shade intolerance smooths]{Fitted smooths for crown competition factor (CCF), purtiy (of larch), and shade intolerance. Error bars represent 2 standard deviations and account for uncertainty in smoothing parameter estimation.}
...
\begin{table}[!h]
\caption[CCF, purity, shade intolerance table]{RMSE, deviance explained, and adjusted $R^2$ values for the base model, and models includeing each species-informed variables}
\label{tab:mixvarsrmse}
\centering
\begin{tabular}{lrrr}
\toprule
Model & RMSE ($\mathrm{cm^2/yr}$) & Deviance explained (\%) & $R^2$\\
\midrule
\cellcolor{gray!6}{Base} & \cellcolor{gray!6}{4.814} & \cellcolor{gray!6}{84.91} & \cellcolor{gray!6}{0.77886}\\
CCF & 4.801 & 84.42 & 0.77597\\
\cellcolor{gray!6}{Purity} & \cellcolor{gray!6}{4.595} & \cellcolor{gray!6}{85.07} & \cellcolor{gray!6}{0.77942}\\
Shade intol. & 4.627 & 85.02 & 0.77946\\
\bottomrule
\end{tabular}
\end{table}
...
The model selection approach applied here produced a BAI model 
fit with smooth functions that resemble previously established functional 
relationships between BAI and each covariate except for BAL. Other functions 
...
from those previously identified \cite{Stage1973b,Monserud1996,Hann1991,Vospernik2020}. 
...
GAMs (and GAMMs) are becoming more prevalent in ecological research for 
various applications to aid in capturing and describing complex non-linear 
relationships that otherwise would require extensive testing to determine an 
assumed model form in parametric approaches \citep{Robinson2011,Adamec2015}. 

\cite{Johnston2019} showed that GAMs were effective in modeling tree growth 
response across different levels of fire severity. 

%% interpret relationships


The GAM models fit estimate BAI conditional on a gamma 
distribution with a log-link function, thus the model terms together are  
considered to have a multiplicative effect on the mean response \citep{Faraway2016}, 
allowing for interpretation of terms as interactive with one another. 


\cite{Vospernik2020} found that only including BAL as a competition measure 
was sufficient in capturing competitive effects. That option was 
excluded here because it was thought important to consider tree-level 
competitive effects in addition to community crowding effects. This allowed for 
more accurate estimates, as BAH showed a large effect on growth estimates. 
Additionally, the use of BAL corroborates with \cite{Contreras2011}, which 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 may be a limitation of the current study. Aspect was selected in the model 
 because it outweighed alternative measures, including a more comprehensive 
 heatload metric, which is based on a combination of slope, aspect, and 
 elevation. Including additional site variables to 
