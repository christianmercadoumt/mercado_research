...
individual tree growth models. The smooths fitted for diameter, 
crown ratio, and BAH are all consistent with well-established 
functional relationships for each term \citep{Wykoff1990,Monserud1996}. 
This is somewhat unsurprising given that all variables were considered because 
they are well-established in tree growth modelling. 
...
characterizations, highlighting a potential advantage of this approach. 
BAL previously has been characterized by an overall 
decreasing growth effect. In this model, it doesn't appear relevant 
until BAL is in excess of 0.8, meaning that western larch growth may not be 
...
competitive effects in addition to community crowding effects. 
The results in this study indicate both levels are pertinent in the western 
larch growth relationship. They may also indicate that western larch has competitive 
...
Allowing this (and other) functional relationships to vary dynamically 
may help uncover effects that may be missed when fitting 
models with \emph{a priori} functional shape assumptions. 
...
careful consideration when fitting models like this. In this study the 
maximum effective degrees of freedom (EDF; defined by k-1) for each smooth chosen was 
9 for DBH , BAL, and BAH, the default value provided by mgcv (k = 10). Although 
the degree of penalization is not necessarily selected through 
specifying this parameter \citep{wood2006generalized}, a higher value of 
k creates more space for a function to vary. In the case of BAH, 
...
effect that, which was backed by a very low p-value. In other cases, such as the 
shade intolerance smooth, further constraints may be necessary to 
provide for a higher confidence in valuable interpretation. 
...
regardless of slope or elevation on a particular aspect.
...
incoming solar radiation \cite{Theobald2015}. However, using map-based metrics 
...
variables as well as with BAH. These inconsistencies highlight that which was 
found by \cite{Froese2007}, noting that there was bias associated with growth prediction 
when site conditions are favorable. Clearly there are tradeoffs associated 
...
modeled may be partially due to the average and maximum tree sizes in this 
...
forms provide mixed results \citep{Weiskittel2011}. Nonetheless, this 
...
scaling issues, thus interpretation is unaffected. 
...
densities are associated with high resource consumption, limiting growth. 
However, compared to the shape of the difference smooth for lodgepole pine, 
...
with Douglas-fir, however, due to the limitations discussed, and a lack of 
...
density, and site productivity \citep{Kimsay2019,Wieskittel2009a,Huber2014}. 
...
the issue of initiating climate adaptation in western larch forests through 
...
larch, however, the growth of other species in the presence of fast-growing 
...
spruce, however, they too suffered from a lack of reliable data. \cite{Xie} 
assessed the height-diameter relationship of \emph{L. olgensis}, and 
found inconsistent results presuming more height growth in mixtures. 
