...
for each predictor, and a \emph{difference smooth} for species-mixture 
\citep{wood2006generalized}: 
...
deviance explained, followed by size (Table \ref{table:rmse}). However, considering 
