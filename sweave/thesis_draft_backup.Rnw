...
parameter for each level of a categorical variable \citep{Pedersen2019}. Thus, 
a separate smoother for each level of a specified categorical 
variable may be fit. Further specification may dictate model fitting where 
one category may be specified as the reference level and fit with a smooth 
representing reference conditions. In this case, each of the remaining levels 
are then fit with respective smooths which 
characterize how the relationship deviates from the reference 
smooth \cite{Pedersen2019,wood2006generalized,zurr2009}. 
...
sub-analyses. The second model, then, was 
...
Selecting which predictors to be included in the model 
...
% Size variables considered were $DBH_{km}$ and $BA_{km}$. 
...
Models were fit in iterations by predictor grouping, 
the order of which was based on previously identified contributions 
of these predictor groups in estimating BAI \citep{Wykoff1990,Monserud1996}. 
Highly correlated same-group variables were compared in alternative models 
to determine which contributed the most to estimating $BAI_{km}$.
For example, alternative models were fit for both tree $DBH_{km}$ 
and $BA_{km}$, selecting whichever best predicted BAI. 
...
finalizing the growth relationship (in the absence of 
...
where $ln$ represents the logarithmic link function, $E(BAI_{km})$ represents the expected 
value of $BAI_{km}$, $\beta_0$ represents an estimated intercept, $\sum_j$ represents summation 
...
When comparing models, a model formulation with the lowest $RMSE_{df}$ 
...
\begin{equation}
\label{eq:ccf}
CCF_{pm} = \sum\limits_{km} TPH_{km} h_s(DBH_{km})
\end{equation}

where $h_s()$ is a function of the $DBH_{km}$ of tree $k$, given its species $s$. 
...
achieved their MCA and canopy closure is achieved, i.e., any value in excess 
...
\begin{equation}
\label{eq:shade}
T_{pm} = \frac{\sum\limits_{k,pm}{\rho_{s} BA_{s_{km}}}}{\sum\limits_{k,pm} BA_{km}}
\end{equation}
...
tree $k$ \citep{Lienard2015}. $T_{pm}$ effectively describes community 
shade-intolerance through a proportion of plot basal area, where a $T_{pm}$ value of 1 
...
accuracy (RMSE) and fit, quantifying 
each predictor's relative improvement to model accuracy. 
% 
% RMSE values were 
...
as shown in figure \ref{fig:sumfig}. Growth increases the most vigorously 
in trees less than 20 cm in $DBH_{km}$ at low densities, where a nonlinear growth 
relationship is apparent. At moderate to higher densities, however, $DBH_{km}$ and 
...
aspects towards northern and eastern aspects, the $DBH_{km}$-$BAI_{km}$ trend appears less 
...
\caption[Stand diameter distributions]{Boxplot distributions of diameter ($DBH_{km}$) by stand, colored by aspect (aspect coded by cardinal direction). The largest trees were concentrated in 2 stands (1401 and 1408). Boxplot whiskers are equal to 1.5 interquartile range.}
\label{fig:ddist}
\end{figure}

%Then presesnt results of correlation and say which predictors were selected by group
Tree $DBH_{km}$ and basal area were highly correlated (0.938) and were 
evaluated in separate models. $DBH_{km}$ as a predictor yielded slightly higher 
...
(BAL, $\mathrm{Dq_{km}}$, and CR), and variables that represent the overall competitive 
environment (QMD, TPH, $BAH_{pm}$, and CCF). Highly collinear variable 
combinations were were TPH-$\mathrm{Dq_{km}}$ (0.732) and BA-QMD (0.684).  %, CCF-BA (0.95)
With $DBH_{km}$ effects included, adding tree-level variables reduced RMSE to 4.50 
...
variables individually, $DBH_{km}$ explained more deviance and reduced RMSE 
...
\hspace{1em}\cellcolor{gray!6}{$DBH_{km}$} & \cellcolor{gray!6}{8} & \cellcolor{gray!6} & \cellcolor{gray!6} \\
\addlinespace[0.3em]
...
$DBH_{km}$, CR, BAL, $BAH_{pm}$, aspect, and a tree-level random effect. 
...
Partial response curves are shown in Figure \ref{fig:basesmooths}. $DBH_{km}$ had an 
...
% \end{subfigure}
\caption[Base model smooths]{Smoothed effects fitted by the selected base model. The top 4 plots show the 1 dimensional smooth effects for diameter at breast height ($DBH_{km}$), tree crown ratio, ratio of basal area larger than a subject (BAL), and plot basal area (BA). Effect scales vary by smooth.}
...
\hhline{~|}
\multicolumn{1}{c|}{\multirow[c|]{-4}{*}{\cellcolor{gray!6} $DBH_{km}$($cm$)}} & \cellcolor{gray!6}{mean} & \cellcolor{gray!6}{13.6} & \cellcolor{gray!6}{9.2} & \cellcolor{gray!6}{12.4}\\
\hhline{-|----}
...
as $DBH_{km}$, symmetry of competition BAL, and stand density BA varied
...
Furthermore, the BAI-$BAH_{pm}$ difference smooth showed a positive difference at 
...
\midrule
\cellcolor{gray!6}{$DBH_{km}$} & \cellcolor{gray!6}{7.89} & \cellcolor{gray!6}{$< 0.001$} & \cellcolor{gray!6}{2.75} & \cellcolor{gray!6}{$< 0.001$} & \cellcolor{gray!6}{2.14} & \cellcolor{gray!6}{0.398}\\
...
predictors in respective models. CCF and $BAH_{pm}$ 
were found to be highly correlated with one-another, so ccf was included 
in its respective model in place of $BAH_{pm}$, as to avoid issues of collinearty  
...
In the absence of $BAH_{pm}$ in the model, CCF displayed a larger effect on tree growth 
...
to the $BAH_{pm}$ smooth in the base model, showing a declining effect from low to high values. 
...
by $BAH_{pm}$ in each of these models, both of these measures have an 
...
- The functional form of $BAH_{pm}$ is similar to previous works
...
more accurate estimates, as $BAH_{pm}$ showed a large effect on growth estimates. 
...
it offered greater improvements to RMSE than $BAH_{pm}$ when slope was also considered 
in the model.  
but these improvements 
were marginal compared to $BAH_{pm}$, as demonstrated in the comparison 
...
--- $BAH_{pm}$ had a magnifying negative effect at high densities
...
-- ccf served well in place of $BAH_{pm}$, but its hard to interpret species 
effects. 
