...
\begin{equation}
\label{eq:bai}
BAI_{km} = \frac{BA_{m+1}-BA_{m}}{t_{m+1}-t_m}
\end{equation}

, where $BA_{km}$ refers to tree $k$ basal area at measurement occasion $m$ at 
year $t$, where $m+1$ indicates a subsequent measurement occasion.
...
(referred to as competition herein) and (iii), characteristics of the site 
where trees are growing. 

Size variables include diameter at breast height $DBH_{km}$ and 
tree basal area $BA_{km}$. 
Competition variables were: plot $p$ number of trees per hectare 
($TPH_{pm}$), plot basal area per hectare ($BAH_{pm}$), 
ratio of plot $BAH_{pm}$ in trees that are larger than an individual tree 
($BAL_{km}$), plot quadratic mean diameter ($QMD_{pm}$), and the ratio of 
tree $DBH_{km}$ to $QMD_{pm}$ ($Dq_{km}$). 
Site variables were percent slope, aspect (between 0 and 360 degrees), 
elevation (in meters), site index 
\citep[an average height of 50-year-old site trees;][]{Reineke1933}, 
and heatload \citep[90 m resolution][]{Theobald2015}, 
where slope, aspect, elevation, and heatload were measured and/or calculated 
for each cluster using GPS data points, pulled from a digital elevation model. 

Mathematical definitions for calculated variables are found in equations 2.2-2.9, 
where $k$ denotes an individual tree, $p$ represents a unique plot, and 
$m$ represents a distinct measurement occasion. 

% \begin{equation}
\begin{align}
BA_{km} &= \pi\frac{DBH_{km}^2}{4} \label{eq:BA}\\[1em]
TPH_{km} &= hCount_{km} \label{eq:treeTPH}\\[1em]
TPH_{pm} &= \sum_{k,pm} TPH_{km} \label{eq:TPH}\\[1em]
BAH_{pm} &= \sum_{k,pm} hBA_{km} \label{eq:BAH}\\[1em]
BAL_{km} &= \frac{BA_{km}}{\sum\limits_{k,pm} BA_{km} > BA_{km}} \label{eq:BAL}\\[1em]
QMD_{pm} &= \frac{BAH_{pm}}{0.0000785TPH_{pm}} \label{eq:QMD}\\[1em]
Dq_{km} &= \frac{DBH_{km}}{QMD_{pm}} \label{eq:Dq}\\[1em]
\end{align}
% \end{equation}
% 0.0000785
...
Models were used to evaluate the effects of different combinations 
of these variables on BAI of western larch trees throughout these data, using 
...
tree data from two measurement plots out of each stand 
(across measurements occasions). A summary of the training data set may be 
viewed in table \ref{tab:sumtab}. 
...
(or "wiggliness") grows higher \citep{Pedersen2019}.
In mgcv, the thin-plate regression spline (TPRS) basis type 
is the default smoother owing to its simple and effective behavior \citep{Wood2003},
...
In this study, it was of interest to compare GAMs for their predictive 
...
training data and model number of residual degrees of freedom (RMSE_{df}), 
and the other which evaluates error using the withheld data, and number of 
observations thereof (RMSE): 

\begin{align}
RMSE_{df} = \sqrt{\frac{\sum_k (\widehat{BAI_{km}} - BAI_{km})^2}{n_{df}}} \label{eq:rmsedf}\\
RMSE = \sqrt{\frac{\sum_k (\widehat{BAI_{km}} - BAI_{km})^2}{n}} \label{eq:rmse}
\end{align}

where $\widehat{BAI_{km}}$ is the model prediction, 
...
and $n$ is the number of observations. $RMSE_{df}$ was used for comparing 
model accuracy across models during model selection, and RMSE was used to 
when determining model validity. 
...
% s to evaluate 
% the independence of data as well as overall homogeneity. 
% 
% to assess for 
...
Three different GAMM analyses were conducted, where each involved fitting 
models to estimate the BAI of individual western larch trees. 
...
model structure was then used as a foundation for the following two model 
sub-analyses, which were fit including same variables. The second model, then, was 
...
alternative in estimate in estimating growth. The following three sections will 
describe how each of these was accomplished. 
...
grouped together based on the categories described above: 
...
freedom to provide an unbiased estimate. 

If estimated concurvity was greater than 0.8 for a particular 
