...
recorded. For small trees tall enough for DBH measurement ($\ge \mathrm{1.4 m}$) and with a DBH below 
...
where $G_{kpm}$ refers to the basal area of tree $k$ in plot $p$ at  
...
\begin{enumerate}
\item plot number of trees per hectare ($TPH$)
...
\item plot basal area per hectare ($BAH$) 

\begin{equation}
BAH_{pm} = \sum_{k} h_{kpm} G_{kpm}
\end{equation}

\item the ratio of plot $BAH$ in trees that are larger than the subject tree ($BAL$) 

\begin{equation}
BAL_{kpm} = \frac{\sum\limits_{k^{\prime}} h_{k^{\prime}pm} G_{k^{\prime}pm}}{BAH_{pm}}
\end{equation}

where the sum is over trees $k^{\prime}$ with DBH larger than the subject tree, 

\item plot quadratic mean diameter ($QMD$) 
...
\item the ratio of tree $DBH$ to $QMD$ ($Dq$) 
...
Western larch site index was calculated at the stand-level as an average value over 
...
tree data from two randomly selected measurement plots out of each stand. An overview of the 
...
Smoothers are composed of a sum of fitted basis functions 
...
parameter for each level of a categorical variable \citep{Pedersen2019}. 
Specifically, one level may be specified as 
...
The other evaluated error using the withheld data and number of 
observations therein ($\mathrm{RMSE_{ext}}$): 

\begin{align}
\mathrm{RMSE_{int}} &= \sqrt{\frac{\sum_m \sum_p \sum_k (\widehat{BAI}_{kpm} - BAI_{kpm})^2}{n_{df}}} \label{eq:rmsedf}\\[1em]
\mathrm{RMSE_{ext}} &= \sqrt{\frac{\sum_m \sum_p \sum_k (\widehat{BAI}_{kpm} - BAI_{kpm})^2}{n}} \label{eq:rmse}
\end{align}
...
\citep{wood2006generalized}, (lying between 0 and 1), 
...
of the model in which it is embedded. The latter of these two options was chosen for 
simplicity, and a model with any estimated concurvity value > 0.8 was not considered. 
...
but that carry no information about community species composition. This initial 
model and its formulation will be referred to herein as SCPt (t refers to 
the inclusion of tree-level random effects). Once identified, 
...
they could effectively capture differences in the growth of western larch more broadly. 
...
value of $BAI$, $B$ represents an estimated tree-level random intercept (t), $x_{ij}$ 
...
open-grown conditions for a given DBH. These maximum crown widths are 
...
\begin{equation}
\label{eq:ccf}
CCF_{pm} = \sum\limits_{k} h_{kpm} C_{kpm} w_s(DBH_{kpm})
\end{equation}
...
achieved their respective MCAs then 100\% crown canopy cover is achieved. Thus, 
a value of 100 theoretically describes the threshold of competition for crown space. 
...
given target species. It follows from the simple logic that if a high proportion of 
...
and $L_{pm}$ denotes the plot larch purity value at measurement $m$ for 
western larch. 
...
ranging between 0 and 1, 1 being very shade-tolerant, and 0 being very 
...
\begin{table}
\caption[Species shade intolerance values]{Shade intolerance values for the PGP species, based on those identified by \cite{Lienard2015}.}
\label{tab:shade}
\centering
...
in terms of predictive accuracy ($\mathrm{RMSE_{ext}}$). Model fit was also evaluated 
