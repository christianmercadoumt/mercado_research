...
plot basal area per hectare ($BAH$) 

\begin{equation}
BAH_{pm} = \sum_{k} h_{kpm} G_{kpm}
\end{equation}

the ratio of plot $BAH$ in trees that are larger than an individual tree ($BAL$) 

\begin{equation}
BAL_{kpm} = \frac{\sum\limits_{k`} h_{k`pm} G_{k`pm}}{BAH_{pm}}
\end{equation}

where $\sum_{k`}$ is over trees $k`$ with DBH larger than a subject $k$, 
...
and the ratio of tree $DBH$ to $QMD$ ($Dq$) 
...
a separate smoother for each level. In particular, model fitting where 
one level may be specified as the reference level and fit with a smooth 
representing reference conditions. Each of the remaining levels 
are then fit with respective smooths that each characterize how the relationship 
for that level deviates from the reference smooth \citep{Pedersen2019,wood2006generalized,zurr2009}. 
...
observations therein ($\mathrm{RMSE_ext}$): 
...
model and its formulation will be referred to herein as SCP. Once identified, 
SCP was then used as a foundation for subsequent analyses. 
The second analysis utilized data only from three specific stand species 
combinations, fit to SCP, and evaluated differences of smooths across these 
combinations. 
% whether and how 
...
% species mixtures. 
The third analysis involved augmenting SCP using three different species-mixing 
...
Selecting which predictors to include within the initial model (M1)
...
0.7 or higher were not considered in a model together. Additionally, 
...
individual trees were then included. This resulted in model SCP, taking the 
following form: 
...
value of $BAI$, $B$ represents an estimated tree-level random intercept, and $x_{ij}$ 
...
SCP was then fit with the species-mixture data, allowing each mixture an offset 
...
for species $s$ and predictor $j$. 
These difference smooths were evaluated 
against the pure larch reference smooth using a hypothesis test. In this test, 
the null hypothesis states that a coefficient in a linear model for a given term 
...
indeed different from zero. P-values were calculated using an F-test, and were  
...
Given that western larch growth may vary across mixtures depending on the characteristics 
of different species within a mixture, it was of interest to capture 
species-mixing effects in a way that is easy to measure or calculate from readily 
...
additional predictor in the SCP model: crown competition factor, the proportion 
...
can impact the growth of that species is low. Alternatively, if a given species 
is surrounded by a high proportion of basal area in other species, then 
the amount that species's growth of the target species may be 
increased. 

Assuming that species-mixing can impact the growth of western larch, 
then the amount of biodiversity in a stand would be be inversely related 
to the amount of area occupied by western larch (or the purity thereof). 
Therefore, as an alternative to CCF, the proportion of basal area occupied by larch 
(purity, herein) was used to evaluate species mixing effects. It is simply calculated as a 
ratio of plot basal area occupied by western larch to total plot basal area: 
