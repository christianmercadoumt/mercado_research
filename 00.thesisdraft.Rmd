...
The following steps outline the modeling workflow that I used: (i) evaluating and estimating potential explanatory variables in the available data set based on established relationships with the dependent variable (BAI~~or height~~), (ii) sequentially selecting predictor variables within pre-determined groups through a novel pragmatic variable selection approach to identify a base-model structure for the GAM, (iii) evaluating random effects due to hierarchical groupings in the sample-design, and finally (iv) assessing effects produced by identified species-mixing variables. 

For (i), explanatory variables were identified based on availability in the PGP data set and whether they could be calculated from the available data. Variables contained in established and widely-used empirical models, such as the diameter growth model in FVS (*CITE*), were considered because, 1. the PGP data collection focused on such variables, and 2. these variables are relatively easy to collect and commonly found in long-term data collection efforts similar to the PGP data. This allows for the potential expansion of these methodologies to other, similar works, given that they produce reliable estimates in this study. Variables considered that were beyond the scope identified above, related primarily to site conditions. **ADD TABLE that lists variables from the ln(DDS) model, as well as variables not in that model in BOLD text?** Once explanatory variables were identified for consideration, they were then grouped into the following categories: tree size, density and competition, site, and species-mixing.  

The following steps outline the pragmatic variable selection approach mentioned in step (ii) above:
...
5. Once selections are made based on the above process, then a concurvity assessment is made within each group. If concurvity is greater than 0.8 for a particular set of terms, *such terms will be scrutinized.* (Johnston et al 2019 uses a very low concurvity cutoff. It oftentimes is arbitrary). **now I'm using the concurvity estimate provided by mgcv, these values are much lower in general - since I'm using the estimate, maybe I should consider dropping my concurvity cutoff to 0.5??? 0.3?? they're more or less arbitrary, but 0.3 would definitely help avoid issues. but is it feasible? I'm investigating.. starting with 0.5, for kicks**
...
Selecting variables within each group posed challenges due to the number of predictors, colinearity and/or concurvity among variables and terms, and the numerous potential interactions between variables within groups. This required a firmly-structured process where co-variate relationships are assessed. 
...
The final base model chosen included DBH, crown ratio, BAL, crown compeition factor (CCF), aspect (represented by an interaction between sin(ASP) and cos(ASP)). 

#### random effects

Random effects were considered in the form of intercepts at each of the following hierarchies, all exclusive, not together: stand, measurement cluster, plot, and tree. Ultimately, accounting for tree-specific effects improved overall model accuracy the best. To aid in reducing model complexity and computation times, more complex representations of nesting was not accounted for here. 
...
1. Wiggliness is hard to control when putting all of these variables together. Tryhing to fine-tune adjust smooth terms is too difficult as the model gets more complex. 
2. Concurvity does not capture everything, and if too heavy of a weight is put on this metric, then model fitting process can become extremely arduous. In this case, concurvity ought to be used as  a rough metric. It's important to acknowledge that these variables do have overlap. 
