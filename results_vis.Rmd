...
fvs.full <- readRDS('data/model_objects.1/fvs.full.re.rds')
```
...
```{r mean dbh over time}
###### BIN THE lines together by color - perhaps just split it into 3 or 4 colors, but keep the individual lines. 
### Change axis titles
### swap to centimeters?
...
mdbh <- pgp_data_all %>% filter(SPECIES_SYMBOL == 'LAOC', TPA_EQUIV.pl != 0) %>% group_by(SETTING_ID, myear) %>% summarise(mean_diam = mean(DIAMETER, na.rm = T))


ggplot(mdbh, aes(x = myear, y = mean_diam, color = SETTING_ID)) + geom_point() + geom_line()
```
...
re.tree.1$deviance
anova.gam(re.tree.1, spmx.re.gam.lf)
...
smod <- spmx.re.gam.lf$model
smod1 <- left_join(smod, bai.spmx.ids)
smod1.pr <- cbind(partial_residuals(spmx.re.gam.lf), smod1$dom.spp.pl.ba)
...
ggplot(lf.smest, aes(x = larch.ba.fraction.pl, y = cm.sq*exp(est))) + 
  ...
  #geom_point(aes(x = larch.ba.fraction.pl, y = smod1.pr$`s(larch.ba.fraction.pl)`, color = factor(smod1.pr$`smod1$dom.spp.pl.ba`)))
#shade tolerance
...
abc <- gam(bai~s(DIAMETER), data = bai.spmx.ids, family = Gamma(link = 'log'), method = 'ML')
abc1 <- gam(I(bai*cm.sq)~s(DIAMETER), data = bai.spmx.ids, family = Gamma(link = 'log'), method = 'ML')
...
```{r}
re.tree.1
d <- summary(nosize.base)
e <- summary(nocomp.base)
f <- summary(nosite.base)



deviance(re.tree.1)
deviance(nosize.base)
deviance(nocomp.base)
deviance(nosite.base)


```

```{r}
plot.gam(re.tree.1, rug = F, pages = 1)

ggplot(bai.spmx.ids, aes(x = DIAMETER)) + geom_histogram() + facet_grid(~split)

my.rmse.2(fvs.full)[[2]]*cm.sq
summary(fvs.full)
```
