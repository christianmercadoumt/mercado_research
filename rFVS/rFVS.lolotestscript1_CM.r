# 1. set directories
fvs_r_code <- "c:/OpenFVS/rFVS/R"  # location of OpenFVS R scripts
fvs_bin <- "c:/OpenFVS/trunk/bin"  # location of OpenFVS binaries
key.dir <- file.path(getwd(),"rfvs_code")        # location of key files that we'll create
lolo.data <- file.path(getwd(),"rFVS")         # location of lolo NF pgp data

## CM - What/where is loloNF???? Is the lolo.data object supposed to be creating such a folder??? Assuming the directory is "mercado_research", there is no "loloNF" to be found
...
#CM edit - Changed qFVSie to FVSie

fvsLoad("FVSie", bin=fvs_bin) # loads ie varient
spp <- as.data.frame(fvsGetSpeciesCodes())
spp$fvs_num <- as.numeric(row.names(spp))


# 4. obtain and format tree data 
# CM - ERROR - lolo.data points to loloNF, but loloNF doesn't exist in the repo/file structure, therefore lolo_stands.Rdata doesn't exist within any such directory
# CM - lolo_stands.Rdata does exist in rFVS. Edited to point to there
