...
gam.s1 <- gam(bai~s(DIAMETER, sp = .7), family = 'Gamma'(link = log), data = bai.size, method = 'ML')

#gam.s1a <- gam(bai~s(DIAMETER, bs = 'tp', k = 8), family = 'Gamma'(link = log), data = bai.size)
#par(mfrow = c(1,1))
#plot.gam(gam.s1a)

gam.s1a <- gam(bai~s(DIAMETER, sp = .7), family = 'Gamma'(link = log), data = bai.size[bai.size$unique_tree_id != 15431,], method = 'ML') #without giant trees

gam.s2 <- gam(bai~s(treeba, sp = 1.1), family = 'Gamma'(link = log), data = bai.size, method = 'ML')

gam.s2a <- gam(bai~s(treeba, sp = 1.1), family = 'Gamma'(link = log), data = bai.size[bai.size$unique_tree_id != 15431,], method = 'ML') #without giant trees


plot.gam(gam.s1, residuals = T)
