---
title: \vspace{-3cm} \begin{normalsize} \textbf{Identifying and evaluating the effects of forest community composition on western larch growth} \end{normalsize}
author: 
  - \begin{normalsize} \textbf{Christian Mercado} \end{normalsize}
  - \begin{normalsize} \textbf{David Affleck} \end{normalsize}
date: \begin{scriptsize} W.A. Franke College of Forestry and Conservation, University of Montana \end{scriptsize}
header-includes:
   - \usepackage{caption}
...
csl: international-union-of-crystallography.csl
...
composition impacts tree growth. This study aims to evaluate 
if and how western larch growth varies across a range of species mixtures using 
a unique long-term data set that is distributed across forest 
...
(ii) to identify if a shade-tolerance based species-mixing metric uniquely 
...
``` 
    ...
    \caption{Effective degrees of freedom (EDF), explained deviance, and RMSE for the base model, base model including larch proportion, and the base model including shade-intolerance. EDF and deviance values are displayed for the full model and broken out by variable groups. Explained deviance values for variable groups show the increase of percent deviance explained upon the inclusion of each group.  Bold values represent full-model metrics and non-bold values represent variable group quantities.}%
    }
  \end{floatrow}
\end{figure}
...
Region 1 developed a network of long-term permanent growth plot clusters (PGPs) 
...
stand consists of 4 randomly selected plot-clusters. 
...
unique western larch trees analyzed over time. 
Tree size varied across stands over time as seen in Fig. 1. 
Data from 2 plots out of each stand were withheld prior to model-fitting for the purpose of validation.

With these data, an individual tree basal area increment (BAI) model was fit using 
...
the model and their contributions to model accuracy and performance was assessed. 
...
species' total stand basal area by shade tolerance.


### Results

Various alternative base model formulations for estimating BAI were considered. 
Ultimately, variables selected were: tree diameter at breast height (DBH), 
crown competition factor (CCF; [@Krajicek1961]), basal area larger than the subject tree (BAL), 
crown ratio, slope, and aspect. Out of all variables selected, 
...
between DBH and BAI.
Both species-mixing terms improved model accuracy and displayed a negative association with 
BAI (Fig.2). However, the partial effect of the shade intolerance index term was 
smaller and less stable than that of the proportion of western larch term, 
and thus may not be as precisely estimated. For both terms, an increase in either variable 
reduced its effect on BAI after accounting for other effects (Table 1). 
Their additions to the base-model did not greatly modify or overlap with the effects of 
any other variables in any important way such as by an inflation of concurvity [@Hastie1990] or perturbation of fitted smooth-relationships.

This work is ongoing, and thus more results are to come. 
For instance, the withheld data will be used to assess the 
ability to generalize the variable effects identified by this model. 
...
community composition varies across species, and evaluate the relative 
impact of species-mixing on tree growth in select forest ecosystems of the inland northwest. 


\begin{figure}[H]
\fcapside{\caption{Partial response curves for each respective species-mixing model term and its effect on BAI. The horizontal dashed line represents a threshold of positive/negative effects.}}
{%
```{r figure2, include = T, fig.align='right', fig.dim=c(4, 2.5)}
limits <- c(0.8, 2.15)
...
f1a <- ggplot(lf.smest, aes(x = larch.ba.fraction.pl, y = exp(est))) + 
  ...
  labs(y = 'Partial effect', x = 'Larch proportion')
#shade tolerance
f1b <- ggplot(st.smest, aes(x = shade.tol.pl, y = exp(est))) +
  geom_ribbon(alpha = 0.2, aes(ymin = exp(lower_ci), ymax = exp(upper_ci), x = shade.tol.pl)) + 
  geom_line() +
