...
mdbh <- pgp_data_all %>% filter(SPECIES_SYMBOL == 'LAOC', TPA_EQUIV.pl == 20) %>% group_by(SETTING_ID, myear) %>% summarise(mean_diam = mean(DIAMETER, na.rm = T)) %>% ungroup() %>% group_by(SETTING_ID) %>% mutate(st.mean.d=mean(mean_diam)) %>% ungroup() %>% 
...
```{r figure1, include=F, out.width='.49\\linewidth', fig.align='left', fig.cap='Average stand diameter for each measurement. Points connected with lines represent these metrics and colored lines show linear trend of diameter over time in three groups based on diameter (red, green, and blue)', fig.ncol=1}


figure1 <- ggplot(mdbh, aes(x = myear, y = mean_diam*2.54, group = SETTING_ID)) + geom_point(alpha = 0.2) + geom_line(alpha = 0.2) + labs(y = 'Stand large tree mean diameter (cm)', x='Measurement year') + geom_smooth(inherit.aes = F, method = 'lm', aes(x = myear, y = mean_diam*2.54, color = cut), se = F, linetype = 'F1', alpha = 0.5) + theme(legend.position = 'none', plot.caption.position = 'plot', plot.caption = element_text(hjust = 0))
figure1
...
```{r table1, include = T, results='asis'}
