...
variables.plot <- function(forestdata, na.rm = TRUE){
  a <- forestdata %>% #call data
    ...
    mutate(ba.pl = sum(BASAL_AREA_EQUIV.pl[TPA_EQUIV != 0], na.rm = na.rm), #plot basal area calculations - sum of BA over plot
           tpa.pl.all = sum(TPA_EQUIV.pl[TPA_EQUIV != 0], na.rm = na.rm), #plot tpa including trees of all size
           tpa.pl.cutoff = sum(TPA_EQUIV.pl[!is.na(DIAMETER) & TPA_EQUIV != 0], na.rm = na.rm), # TPA excluding trees below 4.5'
           bal.pl = map_dbl(BASAL_AREA_EQUIV.pl, ~sum(BASAL_AREA_EQUIV.pl[BASAL_AREA_EQUIV.pl>.x & TPA_EQUIV != 0], na.rm = na.rm)), #plot-level BAL - used basal area/acre here. This step performs the specified calculation across each record within the plot grouping. Thus, this step sums the basal areas for trees based on the condition that they are greater than the basal area of the observed record
           ccf.tree = case_when(
