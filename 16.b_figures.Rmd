...
```{r base model selection figures}
re.tree.1
...
ggplot(sig.diam, 
  ...
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_ribbon(alpha = 0.2) + 
  labs(x = 'DBH (cm)', y = 'Effect', title ='Larch-lodgepole DBH')
...
ggplot(sig.bal, 
  ...
  geom_hline(yintercept = 0, linetype = 'dashed') +
  geom_ribbon(alpha = 0.2) +
  labs(x = 'BAL', y='Effect', title = 'Larch-lodgepole BAL')
...
#   geom_ribbon(alpha = 0.2) +
#   facet_wrap(~facets) + 
#   labs(x = 'DBH (cm)', y='Effect')#+ coord_cartesian(ylim = c(-5,5))
# 
# mixpl.cr <- ggplot(smest.cr, 
#        aes(x = cr, y = est, 
#            ymin = lower, 
#            ymax = upper)) + 
#   geom_line() + 
...
diffs.pl <- arrangeGrob(mixpl.d, mixpl.cr, mixpl.bal, mixpl.ba, nrow = 4)
