...
afunc <- function(sm.data){
  ...
  if('DIAMETER' %in% names(sm.data)){
    p <- ggplot(b, aes(x = DIAMETER*2.54, 
                       y = Effect, 
                       fill = Mixture,
                       color=Mixture)) + 
      geom_line(alpha=0.2) + 
    ...
    b <- b %>% filter(!(ba.pl*(2.47/10.764) > 35 & Mixture == 'Larch-LP'))
    # b <- b %>% mutate(ba.pl = case_when(omixtype %in% c('laoc', 'psme') ~ ba.pl,
                                        # omixtype == 'pico' & ba.pl>30 ~ NA))
    p <- ggplot(b, aes(x = ba.pl*(2.47/10.764), 
                            y = Effect, 
                       fill = Mixture,
                       color=Mixture)) + 
      geom_line(alpha=0.2) + 
