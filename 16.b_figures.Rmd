...
mixpl.aspr <- 
  ...
  geom_label(data = ns.df, aes(x = sin.t, y = cos.t, label = nsew), 
             alpha = 0.3, size = 2) +
  labs(x = 'sin(Aspect)', y = 'cos(Aspect)') +
  scale_fill_gradient2(low = 'blue', high = 'darkred', na.value = 'grey') +
  theme(plot.margin = grid::unit(c(0,0,0,0), 'mm'), 
        panel.background = element_rect(fill = 'white'), 
        panel.grid = element_line(color = 'lightgrey'),
        legend.key.size = unit(0.25, 'cm'),aspect.ratio = 2/(1+sqrt(5)), axis.text = element_blank(),
        axis.title = element_blank(), legend.position = 'left', 
