...
residuals1 <- ggplot(abc, aes(x = 2.54^2*fit, y = devrs)) + 
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'black', size = 0.2) +
  geom_point(alpha = 0.2) + labs(x = bquote('BAI'~(cm^2/yr.)), y = 'Deviance residuals') +
  theme(aspect.ratio = 2/(1+sqrt(5))) + 
...
estimationerr <- ggplot(ft_lp, aes(x = fit.cm, y = bai-fitted)) +
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'black', size = 0.2) +
  geom_point(alpha = 0.2) + 
  labs(x = bquote('BAI'~(cm^2/yr.)), y = 'Error') +
  theme(aspect.ratio = 2/(1+sqrt(5))) + 
  scale_y_continuous(limits = c(-5,5)) +
  geom_smooth(se = F)
