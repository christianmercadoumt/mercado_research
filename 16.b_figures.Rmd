...
ft_lp <- ft_lp %>% mutate(fit.cm = fitted*2.54^2, bai.cm = bai*2.54^2)

estimationerr <- ggplot(ft_lp, aes(x = fit.cm, y = bai.cm-fit.cm)) +
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'black', size = 0.2) +
  geom_point(alpha = 0.2) + 
...
res.err <- arrangeGrob(residuals1, estimationerr, ncol =2)
...
mixpl.d1 <- ggplot(subset(smest.diam, facets == 'Larch-lodgepole'),
  ...
  theme(strip.text = element_blank(),
        aspect.ratio = 2/(1+sqrt(5))) + 
  scale_x_continuous(limits = c(0,20), breaks = c(0,5,10,15,20,25))
