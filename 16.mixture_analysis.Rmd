...
try(mix.diff.gam <- gam(bai~ mixtype + s(DIAMETER) + s(DIAMETER, by = omixtype) + s(cr, k = 9) + s(cr, by = omixtype, k = 9) + s(bal.pl.ratio) + s(bal.pl.ratio, by = omixtype) + s(ba.pl) + s(ba.pl, by = omixtype) +
                      s(asp_sin, asp_cos, k = 20) + s(asp_sin, asp_cos, by = omixtype, k = 20) + s(unique_tree_id, bs = 're'),
                    family = 'Gamma'(link = log), data = o.mixtures.all))
try(saveRDS('C:/Users/cm165878/Box/research_/mix.diff.gam.rds'))
try(saveRDS('data/model_objects.1/mix.diff.gam.rds'))
