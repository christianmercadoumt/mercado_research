...
spp.fun <- function(spp_min, spp_maximum, combined_min){
  ...
  g <- bai.spmx.ids %>% filter(percent_LAOC>0.9) %>% 
...
dddd <- bai.spmx.ids %>% filter(percent_LAOC>.9) %>% 
    mutate(other.pct = percent_LAOC)
summary(dddd$aspect_deg)
```
...
mod.list <- list(psme.gam, pico.gam, pien.gam, laoc.gam)
...
for(i in 1:5){
  par(mfrow = c(2,2))
  plot(laoc.gam, residuals = T, trans = exp, select = i, ylim = c(0,4), shift = 0.0228)
  plot(pico.gam, residuals = T, trans = exp, select = i, ylim = c(0,4), shift = -.15581)
  plot(psme.gam, residuals = T, trans = exp, select = i, ylim = c(0,4), shift = -.426596)
  plot(pien.gam, residuals = T, trans = exp, select = i, ylim = c(0,4), shift = 0.38111221)
}
