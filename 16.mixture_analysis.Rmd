...
# levels(mix.diff.gam$model$omixtype)
mix.diff.nopien <- readRDS('data/model_objects.1/mix.diff.nopien.rds')

plot_difference(mix.diff.nopien, series = DIAMETER, difference = list(omixtype = c('laoc', 'pico')))
plot_difference(mix.diff.nopien, series = DIAMETER, difference = list(omixtype = c('laoc', 'psme')))
plot_difference(mix.diff.nopien, series = cr, difference = list(omixtype = c('laoc', 'psme')))
plot_difference(mix.diff.nopien, series = cr, difference = list(omixtype = c('laoc', 'pico')))
plot_smooths(mix.diff.nopien, series = DIAMETER, comparison = omixtype)

#summary and model check
job::job(import = c(mix.diff.nopien), 
         {library(mgcv) 
...
#resids for groups
modfit <- mix.diff.nopien.m1$fitted.values
modres <- mix.diff.nopien.m1$residuals
xyplot(modres~modfit|o.mixtures.all$omixtype)

#
sum.mixdiff

# concurvity(mix.diff.pien, full = F)
...
newd <- o.mixtures.all %>% mutate(omixtype = factor(omixtype, ordered = F))
no.offset <- bam(bai~ omixtype + s(DIAMETER, by = omixtype) +
                          s(cr, by = omixtype, k = 9) +
                          s(bal.pl.ratio, by = omixtype) +
                          s(ba.pl, by = omixtype) +
                          s(asp_sin, asp_cos, by = omixtype, k = 20)
                          ,
                      family = 'Gamma'(link = log),
                      data = newd, method = 'fREML')


m1 <- bam(bai~ mixtype + s(DIAMETER) + s(DIAMETER, by = omixtype, m = 1) +
                          s(cr, k = 9) + s(cr, by = omixtype, k = 9, m = 1) +
                          s(bal.pl.ratio) + s(bal.pl.ratio, by = omixtype, m=1) +
                          s(ba.pl) + s(ba.pl, by = omixtype,m=1) +
                          s(asp_sin, asp_cos, k = 20,m=1) +
                          s(asp_sin, asp_cos, by = omixtype, k = 20,m=1),
                      family = 'Gamma'(link = log),
                      data = o.mixtures.all, method = 'fREML')

plot.gam(no.offset, scale = 0)
plot.gam(m1, scale = 0)
summary(m1)
...
unique(o.mixtures.nopien$mixtype)
job::job(import = c(o.mixtures.all), {
  library(mgcv)
  mix.diff.gam.nospr.m1 <- bam(bai~ mixtype + s(DIAMETER) + s(DIAMETER, by = omixtype, m=1) +
                          s(cr, k = 9) + s(cr, by = omixtype, k = 9,m=1) +
                          s(bal.pl.ratio) + s(bal.pl.ratio, by = omixtype,m=1) +
                          s(ba.pl) + s(ba.pl, by = omixtype,m=1) +
                          s(asp_sin, asp_cos, k = 20) +
                      ...
                      family = 'Gamma'(link = log),
                      data = o.mixtures.all, method = 'fREML')
  saveRDS(mix.diff.gam.nospr.m1, 'data/model_objects.1/mix.diff.nopien.m1.rds')
})

job::job(import = c(o.mixtures.nopien), {
  library(mgcv)
  mix.diff.gam.nospr.byrand <- bam(bai~ mixtype + s(DIAMETER) + s(DIAMETER, by = omixtype) +
                                     ...
                                     s(asp_sin, asp_cos, by = omixtype, k = 20) +
                                     s(unique_tree_id, bs = 're') +
                                     s(unique_tree_id, by = omixtype, bs = 're'),
                                   family = 'Gamma'(link = log),
                                   data = o.mixtures.nopien, method = 'fREML', discrete = T)
  saveRDS(mix.diff.gam.nospr.byrand, 'data/model_objects.1/mix.diff.nopien.byrandom.rds')
})


job::job(import = c(o.mixtures.nopien), {
  library(mgcv)
  mix.diff.gam.nospr <- bam(bai~ mixtype + s(DIAMETER) + s(DIAMETER, by = omixtype) +
                          s(cr, k = 9) + s(cr, by = omixtype, k = 9) +
                          s(bal.pl.ratio) + s(bal.pl.ratio, by = omixtype) +
                          s(ba.pl) + s(ba.pl, by = omixtype) +
                          s(asp_sin, asp_cos, k = 20) +
                          ...
                          s(unique_tree_id, bs = 're'),
                      family = 'Gamma'(link = log),
                      data = o.mixtures.nopien, method = 'ML')
  saveRDS(mix.diff.gam.nospr, 'data/model_objects.1/mix.diff.nopien_ML.rds')
})


#THIS ONE DIDNT FIT-MORE COEFFICIENTS THAN DATA
job::job(import = c(o.mixtures.nopien), {
  library(mgcv)
  mix.diff.gam.nospr.byrand <- gam(bai~ mixtype + s(DIAMETER) + s(DIAMETER, by = omixtype) +
                                     s(cr, k = 9) + s(cr, by = omixtype, k = 9) +
                                     s(bal.pl.ratio) + s(bal.pl.ratio, by = omixtype) +
                                     s(ba.pl) + s(ba.pl, by = omixtype) +
                                     s(asp_sin, asp_cos, k = 20) +
                                   ...
                                   family = 'Gamma'(link = log),
                                   data = o.mixtures.nopien, method = 'ML')
  saveRDS(mix.diff.gam.nospr.byrand, 'data/model_objects.1/mix.diff.nopien.byrandom_ML.rds')
})

#This one fits the random intercepts with a separate smoothing parameter and variance component for each level of the factor. It may not differ, but it may be more 'correct' to do it this way. 

cr.mod.k9 <- gam(bai~mixtype + s(cr, k = 9) + s(cr, k = 9, by = omixtype), data = o.mixtures.nopien, method = 'REML', family = 'Gamma'(link = log))

cr.mod.k5 <- gam(bai~mixtype + s(cr, k = 5) + s(cr, k = 5, by = omixtype), data = o.mixtures.nopien, method = 'REML', family = 'Gamma'(link = log))
...
mix.diff.gam.nospr.byrand

gam.vcomp(mix.diff.gam.nospr.byrand)
```
