...
pico.gam <- gamm(bai~ s(DIAMETER) + s(cr, k = 7) + s(bal.pl.ratio) + s(ba.pl)+
                    ...
                    random=list(unique_tree_id=~1),
                    family = 'Gamma'(link = log),
                    data = laoc_pico, method = 'ML', verbosePQL = T)
...
ggplot(mixtures.all, aes(x = mixtype, y = laoc_to_other)) + geom_jitter(alpha = 0.5, aes(color = more.larch))
...
plotting_fun <- function(all.data, ylims = c(0, 2), y.brks = seq(0,2,.25)){
  p <- ggplot(all.data) + geom_line(aes(x = DIAMETER, y = bai, color = whichmix)) +
...
bai.bal.a <- predict(laoc.gam$gam, newdata = var.bal, type = 'response', se.fit = T)
aa <- tibble(var.bal, bai = as.vector(bai.bal.a$fit), se = as.vector(bai.bal.a$se.fit))

bai.bal.b <- predict(psme.gam$gam, newdata = var.bal, type = 'response', se.fit = T)
bb <- tibble(var.bal, bai = as.vector(bai.bal.b$fit), se = as.vector(bai.bal.b$se.fit))

bai.bal.c <- predict(pico.gam$gam, newdata = var.bal, type = 'response', se.fit = T)
cc <- tibble(var.bal, bai = as.vector(bai.bal.c$fit), se = as.vector(bai.bal.c$se.fit))

bai.bal.d <- predict(pien.gam$gam, newdata = var.bal, type = 'response', se.fit = T)
...
bai.cr.a <- predict(laoc.gam$gam, newdata = var.cr, type = 'response', se.fit = T)
aaa <- tibble(var.cr, bai = as.vector(bai.cr.a$fit), se = as.vector(bai.cr.a$se.fit))

bai.cr.b <- predict(psme.gam$gam, newdata = var.cr, type = 'response', se.fit = T)
bbb <- tibble(var.cr, bai = as.vector(bai.cr.b$fit), se = as.vector(bai.cr.b$se.fit))

bai.cr.c <- predict(pico.gam$gam, newdata = var.cr, type = 'response', se.fit = T)
ccc <- tibble(var.cr, bai = as.vector(bai.cr.c$fit), se = as.vector(bai.cr.c$se.fit))

bai.cr.d <- predict(pien.gam$gam, newdata = var.cr, type = 'response', se.fit = T)
...
bal.pred.data <- data_slice(psme.gam$gam, 'bal.pl.ratio', data = bal.test, n = 25)
pred <- predict(psme.gam$gam, newdata = bal.pred.data, type = 'link', se.fit = T)
pred <- as.data.frame(pred)
ilink <- family(psme.gam$gam)$linkinv
dddd <- family(psme.gam$gam)
...
#laoc
bal.pred.data_l <- data_slice(laoc.gam$gam, 'bal.pl.ratio', data = bal.test, n = 25)
p1 <- predict(laoc.gam$gam, newdata = bal.pred.data_l, type = 'link', se.fit = T)
p1 <- as.data.frame(p1)
p1 <- transform(p1,
                bai = ilink(fit),
                upr = ilink(fit + se.fit),
